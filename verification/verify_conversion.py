...
def verify_conversion():
    ...
    if not trades_dir.exists():
        print("[ERROR] ERROR: logs/live/trades/ directory does not exist!")
        return False
    ...
    for log_file in sorted(trades_dir.glob('*.log')):
        ...
        try:
            ...
            if not first_line or not first_line.startswith('{'):
                ...
                print(f"  [WARNING]  Skipping {log_file.name} (old format, not JSONL)")
                continue
    ...
    if stats['errors']:
        print(f"[WARNING]  Found {len(stats['errors'])} errors:")
    ...
    print("  [OK] JSONL format (one JSON object per line)")
    print("  [OK] Required fields: timestamp, symbol, trade_type, entry_price, status, order_id")
    print("  [OK] Optional fields: exit_price, profit_usd, additional_info")
    print()
    
    if stats['invalid_trades'] == 0:
        print("[OK] All trades are valid!")
        return True
    else:
        print(f"[WARNING]  {stats['invalid_trades']} invalid trades found")
        return False
