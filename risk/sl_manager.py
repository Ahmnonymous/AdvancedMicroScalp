...
class SLManager:
    ...
    def __init__(self, config: Dict[str, Any], mt5_connector: MT5Connector, order_manager: OrderManager, tp_manager=None):
        """
        ...
        Args:
            ...
            tp_manager: Optional TPManager instance for partial close monitoring
        """
        self.tp_manager = tp_manager  # Store TP manager reference for partial close monitoring
        ...
        # PHASE 1 FIX 1.2: Increase verification delay to 500ms (fixed, no exponential backoff)
        # 200ms was too short, causing verification failures even when SL was correctly applied
        # Fixed delay (no exponential backoff) prevents cascading delays
        self.sl_update_verification_delay = execution_config.get('sl_verification_delay_seconds', 0.5)  # 500ms (increased from 200ms)
        self.use_exponential_backoff = execution_config.get('verification', {}).get('use_exponential_backoff', False)  # PHASE 1 FIX 1.2: Disabled
        ...
        # Profit zone updates configuration (sweet spot + trailing)
        self.profit_zone_updates_enabled = profit_locking_config.get('profit_zone_updates_enabled', True)
        if not self.profit_zone_updates_enabled:
            logger.info(f"[SL_CONFIG] Profit zone updates DISABLED - sweet spot and trailing stops will be skipped")
        ...
        # CRITICAL FIX: Track orphaned tickets that cannot be updated due to lock issues
        # This prevents infinite retry loops hammering the same orphaned lock
        self._orphaned_tickets = set()  # Tickets with orphaned locks (quarantine)
        self._orphaned_ticket_timestamps = {}  # {ticket: timestamp} - when orphaned
        orphaned_timeout_config = self.risk_config.get('orphaned_lock_timeout_seconds', 5.0)
        self._orphaned_ticket_timeout = orphaned_timeout_config  # Quarantine tickets (default 5 seconds, configurable)
        # STEP 5 FIX: Increased lock timeouts to handle contention scenarios
        # Standard timeout: 2.0s (increased from 1.5s per config)
        # Profit-locking timeout: 3.0s (increased from 2.0s)
        config_lock_timeout = self.risk_config.get('lock_acquisition_timeout_seconds', 2.0)
        config_profit_timeout = self.risk_config.get('profit_locking_lock_timeout_seconds', 3.0)
        
        # FIX 3: LIVE RELIABILITY - Use config values directly (no hard-coded caps)
        # Config values: 3.0s standard, 4.0s profit-locking (appropriate for LIVE trading under load)
        # Removing caps allows proper timeout handling under contention
        self._lock_acquisition_timeout = config_lock_timeout  # Use config value directly (3.0s)
        self._profit_locking_lock_timeout = config_profit_timeout  # Use config value directly (4.0s)
        ...
        self._lock_max_hold_time = self.risk_config.get('lock_max_hold_time_seconds', 0.5)  # 500ms max hold time (default, configurable)
        self._lock_force_release_enabled = True  # Enable automatic stale lock recovery
        
        # CRITICAL FIX #3: Lock retry configuration for exponential backoff
        self._lock_retry_max_attempts = self.risk_config.get('lock_retry_max_attempts', 3)  # Default: 3 attempts
        self._lock_retry_backoff_seconds = self.risk_config.get('lock_retry_backoff_seconds', [0.1, 0.3, 0.5])  # Default: 100ms, 300ms, 500ms
        logger.info(f"[LOCK_RETRY_CONFIG] Max attempts: {self._lock_retry_max_attempts} | "
                   f"Backoff: {self._lock_retry_backoff_seconds}")
        ...
        self._fail_safe_tolerance = 0.01  # $0.01 tolerance - don't trigger if within tolerance
        ...
        # CRITICAL FIX: Alerting and monitoring for profit-lock failures
        self._profit_lock_failures = defaultdict(int)  # {ticket: failure_count}
        self._profit_lock_failure_timestamps = {}  # {ticket: [timestamp1, timestamp2, ...]}
        self._profit_lock_alert_threshold = self.risk_config.get('profit_locking', {}).get('alert_failure_threshold', 3)  # Alert after 3 failures
        self._profit_lock_alert_cooldown = {}  # {ticket: last_alert_time} - prevent alert spam
        self._profit_lock_alert_cooldown_seconds = 60.0  # Alert at most once per minute per ticket
        ...
        # FIX 5: Reduced delta threshold from 0.01% (1%) to 0.005% (0.5%) to allow small valid SL changes
        self._min_sl_delta_pct = self.risk_config.get('delta_min_percent', 0.005)  # 0.5% minimum change (configurable, default 0.5%)
        ...
        # PHASE 1 FIX 1.2: Circuit breaker - skip position if 3 consecutive failures
        self._circuit_breaker_threshold = 3  # 3 consecutive failures before activating (PHASE 1 FIX 1.2)
        ...
        self._verification_metrics = {
            ...
            # PHASE 1 FIX 1.2: Additional performance metrics
            'worker_loop_durations': [],  # Worker loop iteration durations (ms)
            'sl_update_timeouts': 0,  # SL update timeout count
            'circuit_breaker_activations': 0,  # Circuit breaker activations
            'position_not_found_count': 0  # Position not found during update count
        }
    ...
    class _AtomicTrackedLock:
        """
        Context manager wrapper for locks that guarantees atomic tracking.
        
        CRITICAL FIX: Ensures lock tracking is set atomically with acquisition
        and cleaned up on exit (even on exceptions), preventing orphaned locks.
        
        Usage:
            lock_acquired, tracked_lock, reason = self._acquire_ticket_lock_with_timeout(ticket)
            if lock_acquired:
                with tracked_lock:
                    # ... do work ...
                # Tracking automatically cleaned up on exit
        """
        def __init__(self, lock: threading.RLock, ticket: int, sl_manager, 
                     thread_id: int, thread_name: str, is_profit_locking: bool, is_trailing: bool):
            self.lock = lock
            self.ticket = ticket
            self.sl_manager = sl_manager
            self.thread_id = thread_id
            self.thread_name = thread_name
            self.is_profit_locking = is_profit_locking
            self.is_trailing = is_trailing
            self._acquired = False
            self._tracking_set = False
        
        def __enter__(self):
            """Enter context - lock already acquired, just set tracking atomically."""
            # Lock was already acquired before this wrapper was created
            # Set tracking atomically
            hold_start = time.time()
            stack_trace = ''.join(traceback.format_stack()[-5:-1])  # Last 4 frames
            
            with self.sl_manager._locks_lock:
                # ATOMIC: Set ALL tracking fields in one critical section
                self.sl_manager._lock_hold_times[self.ticket] = hold_start
                
                if not hasattr(self.sl_manager, '_lock_holders') or self.sl_manager._lock_holders is None:
                    self.sl_manager._lock_holders = {}
                
                self.sl_manager._lock_holders[self.ticket] = {
                    'thread_id': self.thread_id,
                    'thread_name': self.thread_name,
                    'acquired_at': hold_start,
                    'is_profit_locking': self.is_profit_locking,
                    'is_trailing': self.is_trailing,
                    'lock_object': self.lock,
                    'stack': stack_trace,
                    '_wrapper': self  # Store reference for cleanup
                }
                
                if not hasattr(self.sl_manager, '_lock_holder_stack_traces'):
                    self.sl_manager._lock_holder_stack_traces = {}
                self.sl_manager._lock_holder_stack_traces[self.ticket] = stack_trace
            
            self._acquired = True
            self._tracking_set = True
            return self
        
        def __exit__(self, exc_type, exc_val, exc_tb):
            """Exit context - ALWAYS clean up tracking and release lock, even on exceptions."""
            cleanup_error = None
            try:
                # CRITICAL: Clean up tracking BEFORE releasing lock to prevent race conditions
                with self.sl_manager._locks_lock:
                    if self.ticket in self.sl_manager._lock_hold_times:
                        del self.sl_manager._lock_hold_times[self.ticket]
                    if self.ticket in self.sl_manager._lock_holders:
                        del self.sl_manager._lock_holders[self.ticket]
                    if self.ticket in self.sl_manager._lock_holder_stack_traces:
                        del self.sl_manager._lock_holder_stack_traces[self.ticket]
                
                # Release lock (may be reentrant, so release until it fails)
                release_count = 0
                while True:
                    try:
                        self.lock.release()
                        ...
                        # Lock not held anymore (all reentrant acquisitions released)
                        break
                
                if release_count == 0:
                    logger.warning(f"[ATOMIC_LOCK] Ticket {self.ticket} | "
                                 f"Lock was not held when exiting context (may indicate double-release)")
            except Exception as e:
                cleanup_error = e
                logger.error(f"[ATOMIC_LOCK_CLEANUP_ERROR] Ticket {self.ticket} | "
                           f"Error during cleanup: {e}", exc_info=True)
            
            # Don't suppress exceptions - let them propagate
            # But log if cleanup had errors
            if cleanup_error:
                logger.critical(f"[ATOMIC_LOCK] Ticket {self.ticket} | "
                              f"Cleanup error occurred but lock was released")
            
            return False  # Don't suppress exceptions
    
    def _track_profit_lock_failure(self, ticket: int, symbol: str, reason: str):
        """
        Track profit-lock failure and alert if threshold exceeded.
        ...
        self._profit_lock_failures[ticket] += 1
        
        # Track failure timestamps (keep last 10)
        if ticket not in self._profit_lock_failure_timestamps:
            self._profit_lock_failure_timestamps[ticket] = []
        self._profit_lock_failure_timestamps[ticket].append(current_time)
        # Keep only last 10 timestamps
        if len(self._profit_lock_failure_timestamps[ticket]) > 10:
            self._profit_lock_failure_timestamps[ticket] = self._profit_lock_failure_timestamps[ticket][-10:]
        
        failure_count = self._profit_lock_failures[ticket]
        
        # Alert if threshold exceeded
        if failure_count >= self._profit_lock_alert_threshold:
            # Check cooldown to prevent alert spam
            last_alert_time = self._profit_lock_alert_cooldown.get(ticket, 0)
            if (current_time - last_alert_time) >= self._profit_lock_alert_cooldown_seconds:
                self._profit_lock_alert_cooldown[ticket] = current_time
                logger.critical(f"🚨 ALERT: PROFIT-LOCK FAILURE THRESHOLD EXCEEDED | "
                              f"Ticket={ticket} Symbol={symbol} | "
                              f"Failures: {failure_count} (threshold: {self._profit_lock_alert_threshold}) | "
                              f"Reason: {reason} | "
                              f"ACTION REQUIRED: Investigate profit-lock reliability for this trade")
                # Log system event for monitoring
                try:
                    system_event_logger.systemEvent("PROFIT_LOCK_FAILURE_ALERT", {
                        "ticket": ticket,
                        "symbol": symbol,
                        "failure_count": failure_count,
                        "threshold": self._profit_lock_alert_threshold,
                        "reason": reason,
                        "timestamp": datetime.now().isoformat()
                    })
                except Exception as e:
    ...
    def _acquire_ticket_lock_with_timeout(self, ticket: int, is_profit_locking: bool = False, force_non_blocking_first: bool = False, is_trailing: bool = False) -> Tuple[bool, Optional['SLManager._AtomicTrackedLock'], Optional[str]]:
        """
        ...
        FIX 1: Lock tracking is atomic - all holder info is set immediately after acquisition.
        FIX 5: Logging verification - successful acquisitions are logged with full holder info.
        ...
        Args:
            ...
            is_trailing: If True, this is a trailing stop update (needs priority and aggressive stale lock release)
        
        Returns:
            (success, lock, reason) tuple. If success is False, lock is None and reason explains why.
            Lock is an RLock (reentrant) - can be used with 'with lock:' context manager.
            After using 'with lock:', call _release_ticket_lock() to clean up tracking.
        """
        # CRITICAL FIX: Check if ticket is quarantined (orphaned lock)
        # FIX 3: Use shorter quarantine for emergency SL (first eligible updates)
        # Emergency SL: 1s quarantine, Normal updates: 5s quarantine
        is_emergency_sl = force_non_blocking_first  # First eligible updates are emergency
        quarantine_timeout = 1.0 if is_emergency_sl else self._orphaned_ticket_timeout
        
        if ticket in self._orphaned_tickets:
            quarantine_time = self._orphaned_ticket_timestamps.get(ticket, 0)
            elapsed_quarantine = time.time() - quarantine_time
            if elapsed_quarantine > quarantine_timeout:
                logger.info(f"[ORPHANED_LOCK_QUARANTINE_EXPIRED] Ticket {ticket} | "
                          f"Quarantine expired ({elapsed_quarantine:.1f}s > {quarantine_timeout:.1f}s) | "
                          f"Emergency: {is_emergency_sl} | Attempting lock acquisition")
                self._orphaned_tickets.discard(ticket)
                self._orphaned_ticket_timestamps.pop(ticket, None)
            else:
                remaining = quarantine_timeout - elapsed_quarantine
                logger.warning(f"[ORPHANED_LOCK_QUARANTINE] Ticket {ticket} | "
                             f"Quarantined due to orphaned lock | "
                             f"Remaining: {remaining:.1f}s | "
                             f"Emergency: {is_emergency_sl} | "
                             f"Timeout: {quarantine_timeout:.1f}s")
                return False, None, f"Ticket quarantined due to orphaned lock (retry in {remaining:.1f}s)"
            ...
            try:
                if lock.acquire(blocking=False):
                    ...
                    if holder_info is None:
                        # STEP 1 FIX: Force-release orphaned locks when tracking mismatch detected
                        logger.warning(f"[LOCK_TRACKING_MISMATCH] Ticket {ticket} | "
                                     f"Lock is held but tracking is missing | "
                                     f"ORPHANED LOCK DETECTED - Force releasing...")
                        try:
                            # Try to acquire and immediately release to reset lock state
                            if lock.acquire(blocking=False):
                                lock.release()
                                logger.info(f"[ORPHANED_LOCK_RECOVERED] Ticket {ticket} | Lock force-released successfully")
                                lock_actually_held = False  # Lock is now free
                            else:
                                # Lock is held by another thread - log and quarantine
                                logger.error(f"[ORPHANED_LOCK_HELD] Ticket {ticket} | "
                                           f"Lock held by external thread - quarantining ticket")
                                self._orphaned_tickets.add(ticket)
                                self._orphaned_ticket_timestamps[ticket] = time.time()
                                # Continue to attempt acquisition with timeout
                        except Exception as e:
                            logger.error(f"[ORPHANED_LOCK_RECOVERY_FAILED] Ticket {ticket} | Error: {e}", exc_info=True)
                            # Quarantine ticket to prevent infinite retry loops
                            self._orphaned_tickets.add(ticket)
                            self._orphaned_ticket_timestamps[ticket] = time.time()
            except Exception as e:
        ...
        # CRITICAL FIX #1: Adaptive timeout based on operation priority
        # Profit-locking and trailing stops get longer timeout (critical operations)
        # Standard SL updates get reduced timeout (1.0s max) for faster retry cycles and reduced contention
        if is_profit_locking or is_trailing:
            base_timeout = self._profit_locking_lock_timeout  # 6.0 seconds for critical operations
        else:
            # CRITICAL FIX: Reduce timeout for non-critical updates to 1.0s (was 2.0s)
            # This enables faster retry cycles and reduces lock contention
            # Shorter timeout = faster failure detection = more retries = better throughput
            base_timeout = min(self._lock_acquisition_timeout, 1.0)  # Max 1.0 seconds (reduced from 2.0s)
            
            # CRITICAL: If lock has been held > 1.0s, use even shorter timeout (0.5s)
            # This prevents waiting on stale locks
            with self._locks_lock:
                if ticket in self._lock_hold_times:
                    hold_duration = time.time() - self._lock_hold_times[ticket]
                    if hold_duration > 1.0:  # Lock held > 1s is likely stale
                        base_timeout = 0.5  # Aggressive timeout for stale locks
                        logger.warning(f"[STALE_LOCK_TIMEOUT] Ticket {ticket} | "
                                     f"Lock held for {hold_duration:.2f}s | "
                                     f"Using aggressive timeout: {base_timeout}s")
        
        # CRITICAL FIX #3: Exponential backoff with configurable delays
        # Use instance variables initialized from config
        retry_backoff_seconds = self._lock_retry_backoff_seconds
        retries = self._lock_retry_max_attempts
        ...
        for attempt in range(retries):
            ...
            if attempt == 0 or force_non_blocking_first:
                ...
                # Subsequent attempts: use timeout with exponential backoff
                timeout = base_timeout  # Use base timeout (2.0s for standard, 6.0s for profit-locking)
                acquired = lock.acquire(timeout=timeout)
            
            acquisition_time = (time.time() - attempt_start) * 1000  # Convert to ms
            
            if acquired:
                # CRITICAL FIX: Use atomic tracked lock wrapper to guarantee tracking
                # Lock is already acquired, now wrap it for atomic tracking
                thread_id = threading.current_thread().ident
                thread_name = threading.current_thread().name
                
                # Create tracked lock wrapper - tracking will be set in __enter__
                tracked_lock = self._AtomicTrackedLock(
                    lock, ticket, self, thread_id, thread_name, is_profit_locking, is_trailing
                )
                
                # Enter context to set tracking atomically
                tracked_lock.__enter__()
                
                # Log successful acquisition
                logger.info(f"🔒 [LOCK_ACQUIRED] Ticket {ticket} | Thread: {thread_name}({thread_id}) | "
                           ...
                           f"Trailing: {is_trailing} | Lock object: {id(lock)}")
                
                return True, tracked_lock, None
            
            # Lock acquisition failed - apply exponential backoff before retry (only if not last attempt)
            if not acquired:
                if attempt < retries - 1:
                    backoff = retry_backoff_seconds[min(attempt, len(retry_backoff_seconds) - 1)]
                    total_wait_time += backoff
                    
                    # CRITICAL: For profit-locking/trailing, use shorter backoff to reduce delay
                    if is_profit_locking or is_trailing:
                        backoff = backoff * 0.5  # Half backoff for critical operations
                    
                    time.sleep(backoff)
                    logger.debug(f"[LOCK_RETRY] Ticket {ticket} | Attempt {attempt+1}/{retries} | "
                                f"Backoff: {backoff*1000:.0f}ms | Total wait: {total_wait_time*1000:.0f}ms | "
                                f"{'Profit-locking priority' if is_profit_locking else 'Standard'}")
                    continue  # Continue to next retry attempt
                else:
                    # Last attempt failed - log final error and process failure
                    last_error = f"Lock acquisition failed after {retries} attempts (total wait: {total_wait_time*1000:.0f}ms)"
                    logger.warning(f"[LOCK_ACQUISITION_FAILED] Ticket {ticket} | {last_error}")
            
            # Lock acquisition failed - check current holder (only after all retries exhausted)
            if not acquired:
                with self._locks_lock:
                    holder_info = self._lock_holders.get(ticket)
                    if ticket in self._lock_hold_times:
                        hold_duration = time.time() - self._lock_hold_times[ticket]
                        # CRITICAL FIX #3: Ensure we always have proper holder info - eliminate "Unknown"
                        if holder_info:
                            holder_thread_id = holder_info.get('thread_id')
                            holder_thread = holder_info.get('thread_name', f'Thread-{holder_thread_id}' if holder_thread_id else 'Unknown')
                            holder_stack = holder_info.get('stack', 'N/A')
                            # CRITICAL: Verify lock_object is stored
                            if 'lock_object' not in holder_info:
                                logger.error(f"[LOCK_TRACKING_ERROR] Ticket {ticket} | lock_object missing from holder_info! | "
                                           f"Thread: {holder_thread} | This should never happen")
                                # Try to get lock from _get_ticket_lock as fallback
                                holder_info['lock_object'] = self._get_ticket_lock(ticket)
                        else:
                            # No holder info - this means lock is held but not tracked (shouldn't happen)
                            logger.error(f"[LOCK_TRACKING_ERROR] Ticket {ticket} | Lock held but holder_info is None! | "
                                       f"Hold duration: {hold_duration:.2f}s | This indicates a tracking bug")
                            holder_thread_id = None
                            holder_thread = 'Untracked'
                            holder_stack = 'N/A'
                        
                        # CRITICAL FIX: Check if holder thread is dead
                        ...
                        if holder_thread_id is not None:
                            try:
                                ...
                                if holder_thread_id not in active_thread_ids:
                                    ...
                                    logger.critical(f"🔓 [ORPHANED_LOCK_DETECTED] Ticket {ticket} | "
                                                  f"Holder thread {holder_thread} (ID: {holder_thread_id}) is DEAD | "
                                                  f"Lock held for {hold_duration:.2f}s | Attempting recovery...")
                                    # Attempt to recover orphaned lock (clear tracking)
                                    self._recover_orphaned_lock(ticket, lock, holder_thread_id)
                                    # Note: Python RLock cannot be force-released from another thread.
                                    # The lock will remain held until garbage collection.
                                    # We've cleared tracking, so next attempt won't see "No holder info"
                                    # but will still timeout if lock isn't released.
                                    # Quarantine ticket to prevent infinite retries
                                    logger.error(f"⚠️ [ORPHANED_LOCK_QUARANTINE] Ticket {ticket} | "
                                               f"Tracking cleared but lock may remain held - ticket quarantined")
                                    self._orphaned_tickets.add(ticket)
                                    self._orphaned_ticket_timestamps[ticket] = time.time()
                                    return False, None, f"Lock held by dead thread {holder_thread} (ID: {holder_thread_id}) - ticket quarantined"
                            except Exception as e:
                                logger.debug(f"Error checking thread alive status: {e}")
                        
                        last_error = f"Lock held by {holder_thread} for {hold_duration:.2f}s"
                        
                        # CRITICAL FIX: Aggressive stale lock force-release using configurable threshold
                        # Use config threshold (default 200ms) for all operations
                        # Trailing stops and profit locks get priority (same threshold but faster detection)
                        stale_threshold = self._stale_lock_threshold  # Use configurable threshold (default 200ms)
                        
                        # Force release stale locks if held > threshold OR if holder thread is dead
                        if holder_thread_dead or hold_duration > stale_threshold:
                            reason = "dead thread" if holder_thread_dead else f"held for {hold_duration:.2f}s (threshold: {stale_threshold}s)"
                            logger.critical(f"🔓 STALE LOCK DETECTED: Ticket {ticket} | "
                                          f"Reason: {reason} | "
                                          f"Holder: {holder_thread} | "
                                          f"{'TRAILING PRIORITY' if is_trailing else 'Standard'} | "
                                          f"Attempting force release...")
                            
                            # CRITICAL FIX: For trailing stops, force-release by preempting non-trailing locks
                            if is_trailing and not holder_thread_dead:
                                # Check if holder is doing non-trailing operation
                                holder_is_trailing = holder_info.get('is_trailing', False) if holder_info else False
                                if not holder_is_trailing:
                                    logger.critical(f"🚨 TRAILING PREEMPTION: Ticket {ticket} | "
                                                  f"Trailing stop preempting non-trailing lock held by {holder_thread} | "
                                                  f"Hold duration: {hold_duration:.2f}s")
                            
                            # CRITICAL FIX #2: Actually force-release the lock object, not just remove tracking
                            # Get the lock object from holder info - MUST be stored for force-release
                            holder_lock_object = holder_info.get('lock_object') if holder_info else None
                            
                            # Use holder's lock object if available, otherwise use current lock
                            lock_to_release = holder_lock_object if holder_lock_object is not None else lock
                            
                            # Try to force release by removing from tracking AND releasing lock
                            if ticket in self._lock_hold_times:
                                del self._lock_hold_times[ticket]
                            
                            # CRITICAL FIX: Try to actually release the lock if we have the object
                            force_released = False
                            if holder_lock_object is not None or lock is not None:
                                try:
                                    # Try to release the lock if it's held by dead thread or held too long
                                    # Note: This is risky but necessary for deadlock recovery
                                    if holder_thread_dead:
                                        # For dead threads, we can't safely release, but we can remove tracking
                                        # The lock will be garbage collected when thread dies
                                        logger.critical(f"[FORCE_RELEASED_STALE_LOCK] 🔓 DEAD THREAD LOCK: Ticket {ticket} | "
                                                      f"Removing tracking for dead thread {holder_thread} | "
                                                      f"Lock will be garbage collected")
                                        force_released = True  # Consider it released since thread is dead
                                    else:
                                        # For active threads, try to acquire and release to break stale hold
                                        max_force_release_attempts = 5 if is_trailing else 3
                                        for force_attempt in range(max_force_release_attempts):
                                            if lock_to_release.acquire(blocking=False):
                                                lock_to_release.release()
                                                force_released = True
                                                logger.critical(f"[FORCE_RELEASED_STALE_LOCK] STALE LOCK FORCE RELEASED: Ticket {ticket} | "
                                                              f"Reason: {reason} | Attempt: {force_attempt+1}/{max_force_release_attempts} | "
                                                              f"Holder: {holder_thread} | Hold duration: {hold_duration:.2f}s | "
                                                              f"{'TRAILING PRIORITY' if is_trailing else 'Standard'}")
                                                break
                                            elif force_attempt < max_force_release_attempts - 1:
                                                time.sleep(0.01)  # 10ms between attempts
                                except Exception as e:
                                    logger.warning(f"Error force-releasing lock for ticket {ticket}: {e}", exc_info=True)
                            else:
                                logger.warning(f"[FORCE_RELEASED_STALE_LOCK] WARNING: No lock object available for ticket {ticket} | "
                                             f"Cannot force-release, only removing tracking")
                            
                            # Remove holder tracking AFTER attempting force-release
                            if ticket in self._lock_holders:
                                del self._lock_holders[ticket]
                            
                            if not force_released and not holder_thread_dead:
                                logger.warning(f"STALE LOCK ACTIVE: Ticket {ticket} | "
                                            f"Lock is held by active thread {holder_thread}, removed from tracking | "
                                            f"{'TRAILING BLOCKED' if is_trailing else 'Standard'} | "
        ...
        with self._locks_lock:
            ...
            # CRITICAL FIX #3: Ensure we always have proper holder info - eliminate "Unknown"
            if holder_info:
                holder_thread = holder_info.get('thread_name', f'Thread-{holder_info.get("thread_id", "Unknown")}')
                holder_stack = holder_info.get('stack', 'N/A')
            else:
                holder_thread = 'No holder info'
                holder_stack = 'N/A'
    ...
    def _cleanup_lock_tracking(self, ticket: int):
        """
        Clean up lock tracking without releasing the lock.
        
        FIX 2: This is used after 'with lock:' context manager to clean up tracking
        without double-releasing the lock. The lock is already released by the context manager.
        """
        with self._locks_lock:
            if ticket in self._lock_hold_times:
                del self._lock_hold_times[ticket]
    ...
    def _recover_orphaned_lock(self, ticket: int, lock: threading.RLock, dead_thread_id: int) -> bool:
        """
        Attempt to recover a lock held by a dead thread.
        
        FIX 1: Orphaned Lock Force Release
        - Verify thread is dead
        - Force release by acquiring and immediately releasing (works for RLock when holder thread is dead)
        - Clean up tracking state after release
        
        Args:
            ticket: Position ticket number
            lock: The lock object
            dead_thread_id: ID of the dead thread that was holding the lock
        
        Returns:
            True if lock was successfully recovered, False otherwise
        """
        try:
            # Verify thread is dead
            active_thread_ids = {t.ident for t in threading.enumerate()}
            if dead_thread_id in active_thread_ids:
                logger.warning(f"[ORPHANED_LOCK_RECOVERY] Ticket {ticket} | "
                             f"Thread {dead_thread_id} is actually alive - skipping recovery")
                return False
            
            # FIX 1: Force release by acquiring and immediately releasing
            # This works for RLock when holder thread is dead
            try:
                ...
                if acquired:
                    ...
                    logger.info(f"[ORPHANED_LOCK_RECOVERED] Ticket {ticket} | "
                              f"Lock force-released successfully for dead thread {dead_thread_id} | "
                              f"Tracking cleaned up")
                    return True
                else:
                    # Lock is still held - may be held by another active thread
                    logger.warning(f"[ORPHANED_LOCK_RECOVERY] Ticket {ticket} | "
                                 f"Could not acquire lock (may be held by another thread) | "
                                 f"Dead thread: {dead_thread_id}")
                    # Still clear tracking to prevent stale state
                    with self._locks_lock:
                        if ticket in self._lock_hold_times:
                            del self._lock_hold_times[ticket]
                        if ticket in self._lock_holders:
                ...
                logger.error(f"[ORPHANED_LOCK_RECOVERY] Ticket {ticket} | "
                            f"Failed to force release: {e}", exc_info=True)
            ...
            logger.error(f"[ORPHANED_LOCK_RECOVERY_ERROR] Ticket {ticket} | "
                        f"Error during recovery: {e}", exc_info=True)
            return False
    ...
    def _release_ticket_lock(self, ticket: int, lock: threading.Lock):
        ...
        FIX 3: Runtime assertion - warns if lock hold time exceeds max_hold_time.
        This ensures locks are released promptly and prevents blocking.
        """
        hold_duration = 0.0
        ...
        with self._locks_lock:
            if ticket in self._lock_hold_times:
                ...
                # FIX 3: Runtime assertion - warn if lock held too long
                # This is a warning, not a hard failure, to allow for MT5 operations
                if hold_duration > max_hold_time_ms:
                    logger.warning(f"[LOCK_HOLD_TIME_VIOLATION] Ticket {ticket} | "
                                 f"Lock held for {hold_duration:.1f}ms (max: {max_hold_time_ms:.0f}ms) | "
                                 f"Exceeded by {hold_duration - max_hold_time_ms:.1f}ms | "
                                 f"Thread: {thread_name}({thread_id}) | "
                                 f"This may cause lock contention - ensure locks are released immediately after MT5 operations")
                del self._lock_hold_times[ticket]
            if ticket in self._lock_holders:
                holder_info = self._lock_holders[ticket]
                is_profit_locking = holder_info.get('is_profit_locking', False)
                is_trailing = holder_info.get('is_trailing', False)
                del self._lock_holders[ticket]
            if ticket in self._lock_holder_stack_traces:
                del self._lock_holder_stack_traces[ticket]
        
        # BLOCKER #1 FIX: Handle _AtomicTrackedLock context manager properly
        # If lock is a context manager (_AtomicTrackedLock), it's already released by __exit__
        # Only release if it's a direct threading.Lock/RLock
        if isinstance(lock, self._AtomicTrackedLock):
            # _AtomicTrackedLock is a context manager - lock is already released by __exit__
            # Just verify it was properly released (no-op, tracking already cleaned up)
            pass
        elif hasattr(lock, 'release'):
            # Standard threading.Lock or RLock - release it
            lock.release()
        elif hasattr(lock, '__exit__'):
            # Another context manager - call __exit__ to release
            lock.__exit__(None, None, None)
        elif hasattr(lock, 'lock'):
            # Wrapper with underlying lock - release underlying lock
            lock.lock.release()
        else:
            logger.warning(f"[LOCK_RELEASE] Ticket {ticket} | "
                         f"Unknown lock type: {type(lock)} | "
                         f"Cannot release - may cause lock leak")
        ...
        with self._locks_lock:
            for ticket, acquisition_time in list(self._lock_hold_times.items()):
                ...
                if hold_duration > self._lock_watchdog_interval:
                    ...
                    # FIX 3: Stale lock force-release - safely release underlying RLock
                    if self._lock_force_release_enabled and hold_duration > self._lock_max_hold_time:
                        # Get lock object from holder_info if available, otherwise from ticket_locks
                        holder_info = self._lock_holders.get(ticket)
                        lock_to_release = None
                        if holder_info and holder_info.get('lock_object'):
                            lock_to_release = holder_info.get('lock_object')
                        elif ticket in self._ticket_locks:
                            lock_to_release = self._ticket_locks[ticket]
                        ...
                        if lock_to_release:
                            ...
                            try:
                                if lock_to_release.acquire(blocking=False):
                                    ...
                                    logger.critical(f"🔓 FORCE RELEASED STALE LOCK: Ticket {ticket} | Lock held for {hold_duration:.2f}s (threshold: {self._lock_max_hold_time}s) | "
                                                  f"Holder: {holder_info.get('thread_name', 'Unknown') if holder_info else 'Unknown'}")
                                    # Log system event
                                ...
                                logger.warning(f"Error force-releasing stale lock for ticket {ticket}: {e}", exc_info=True)
                                # Still remove from tracking even if force-release failed
                                if ticket in self._lock_hold_times:
                                    del self._lock_hold_times[ticket]
                                if ticket in self._lock_holders:
                                    del self._lock_holders[ticket]
                            ...
                            # No lock object available - just remove tracking
                            logger.warning(f"[WARNING] STALE LOCK ACTIVE: Ticket {ticket} | Lock held for {hold_duration:.2f}s | "
                                        f"No lock object available, removing from tracking")
                            if ticket in self._lock_hold_times:
                                del self._lock_hold_times[ticket]
                            if ticket in self._lock_holders:
                                del self._lock_holders[ticket]
    ...
    def _adjust_sl_for_broker_constraints(self, target_sl: float, current_sl: float,
        ...
        if stops_level > 0:
            ...
            if order_type == 'BUY':
                ...
                # CRITICAL FIX: Only adjust if target SL is TOO CLOSE (above min_allowed_sl)
                # If target SL is too far (below min_allowed_sl), that's OK - don't move it closer
                # Moving SL closer defeats the purpose of stop loss protection
                if target_sl > min_allowed_sl:
                    # Target is too close - adjust to minimum allowed distance
                    target_sl = min_allowed_sl
                # If target_sl < min_allowed_sl, it's fine - it's further away (more protective)
            else:  # SELL
                # For SELL: SL must be at least min_distance above current ASK
                min_allowed_sl = current_ask + min_distance
                # CRITICAL FIX: Only adjust if target SL is TOO CLOSE (below min_allowed_sl)
                # If target SL is too far (above min_allowed_sl), that's OK - don't move it closer
                if target_sl < min_allowed_sl:
                    # Target is too close - adjust to minimum allowed distance
                    target_sl = min_allowed_sl
                # If target_sl > min_allowed_sl, it's fine - it's further away (more protective)
        
        # CRITICAL FIX: Only ensure SL respects stops_level, don't move it closer unnecessarily
        # The previous logic was moving SL closer to market when it should stay further away
        # This section should only validate, not adjust (adjustment already done above)
        if order_type == 'BUY':
            # BUY SL must be below current BID (validation only)
            if target_sl >= current_bid:
                # This is invalid - SL cannot be at or above BID for BUY
                # Calculate minimum distance based on stops_level
                if stops_level > 0:
                    min_distance = stops_level * point
                    target_sl = current_bid - min_distance
                else:
                    # Fallback: at least 1 pip below
                    target_sl = current_bid - (point * 10)
            # If target_sl < current_bid, it's valid - no adjustment needed
        else:  # SELL
            # SELL SL must be above current ASK (validation only)
            if target_sl <= current_ask:
                # This is invalid - SL cannot be at or below ASK for SELL
                # Calculate minimum distance based on stops_level
                if stops_level > 0:
                    min_distance = stops_level * point
                    target_sl = current_ask + min_distance
                else:
                    # Fallback: at least 1 pip above
                    target_sl = current_ask + (point * 10)
            # If target_sl > current_ask, it's valid - no adjustment needed
    ...
    def _enforce_strict_loss_limit(self, position: Dict[str, Any]) -> Tuple[bool, str, Optional[float]]:
        """
        Enforce strict -$3.00 stop-loss for losing trades.
        
        CRITICAL FIX: Prevents early closures by ensuring SL is set correctly.
        Also checks minimum trade duration to prevent micro-loss exits.
        ...
        # Apply SL update using new lock-minimal path:
        # 1) Prepare outside the lock (validation + adjustment)
        # 2) Acquire ticket lock briefly
        # 3) Call _execute_sl_modify_only inside the lock
        should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
            ticket, symbol, target_sl, -self.max_risk_usd, "Strict loss enforcement (-$2.00)"
        )

        if not should_proceed:
            logger.error(f"STRICT LOSS PREP FAILED: {symbol} Ticket {ticket} | Reason: {error_reason}")
            return False, f"Strict loss preparation failed: {error_reason}", None

        final_sl_price = adjusted_sl_price if adjusted_sl_price is not None else target_sl

        # Acquire ticket lock with standard timeout (not profit-locking, not trailing)
        lock_acquired, lock, lock_reason = self._acquire_ticket_lock_with_timeout(
            ticket, is_profit_locking=False, force_non_blocking_first=False, is_trailing=False
        )
        if not lock_acquired or lock is None:
            logger.error(f"STRICT LOSS LOCK FAILED: {symbol} Ticket {ticket} | Reason: {lock_reason}")
            return False, f"Strict loss lock acquisition failed: {lock_reason}", None

        # Perform minimal MT5.modify_order() call inside the lock
        # CRITICAL FIX: Lock wrapper handles tracking cleanup automatically
        with lock:
            success = self._execute_sl_modify_only(ticket, symbol, final_sl_price)

        if success:
            # CRITICAL FIX: Verify with exponential backoff to handle MT5 broker latency
            # MT5 may return success but SL not yet applied - use progressive verification delays
            verification_delays = [0.3, 0.5, 0.8]  # Progressive delays: 300ms, 500ms, 800ms
            verify_effective_sl = None
            ...
            for delay in verification_delays:
                ...
                if verify_position:
                    ...
                    tracer.trace(
                        ...
                        target_sl=final_sl_price
                    )
                    return True, f"Strict loss enforcement (-${self.max_risk_usd:.2f})", final_sl_price
                else:
                    logger.warning(
                        f"STRICT LOSS VERIFICATION FAILED: {symbol} Ticket {ticket} | "
                    ...
                    # All verification attempts failed - but MT5 returned success
                    # Trust MT5's response and assume SL was applied (broker may have delay)
                    return True, f"Strict loss enforcement applied (verification shows ${verify_effective_sl:.2f}, may be broker delay)", final_sl_price
            
            # If we get here, all verification attempts failed (position not found)
            logger.warning(f"Cannot verify strict loss SL for {symbol} Ticket {ticket} | Position not found after all attempts")
            tracer.trace(
                function_name="SLManager._enforce_strict_loss_limit",
                expected=f"Enforce strict loss -$2.00 for {symbol} Ticket {ticket}",
            ...
            # Still return success since MT5 reported success
            return True, f"Strict loss enforcement applied (position not found for verification)", final_sl_price

        # CRITICAL: If normal update failed, try emergency fallback immediately
        logger.error(
            f"STRICT LOSS UPDATE FAILED: {symbol} Ticket {ticket} | "
            f"Target SL: {final_sl_price:.5f} | Attempting emergency fallback..."
        )
        ...
        try:
            ...
            if fresh_tick:
                ...
                if emergency_success:
                    logger.critical(
                        f"EMERGENCY STRICT SL APPLIED: {symbol} Ticket {ticket} | "
                        f"Emergency SL: {emergency_sl:.5f} | Direct modification succeeded"
                    )
                    return True, f"Emergency strict loss enforcement (-${self.max_risk_usd:.2f})", emergency_sl

                logger.critical(
                    f"EMERGENCY STRICT SL FAILED: {symbol} Ticket {ticket} | "
                    f"Emergency SL: {emergency_sl:.5f} | Direct modification failed"
                )
            ...
            logger.critical(
                f"EMERGENCY STRICT SL EXCEPTION: {symbol} Ticket {ticket} | "
    ...
    def _is_feature_disabled_by_kill_switch(self, feature_name: str) -> bool:
        ...
        # Use unified SL update pipeline (prepare + minimal modify)
        should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
            ticket,
            position.get('symbol', ''),
            target_sl,
            0.0,
            "Break-even SL (profit > $0 but < $0.03 for 2+ seconds)",
        )

        if not should_proceed:
            return False, f"Break-even blocked: {error_reason}", None
        ...
        if not lock_acquired or lock is None:
            logger.warning(
                f"[BREAK_EVEN_LOCK_FAILED] Ticket={ticket} Symbol={position.get('symbol', '')} | "
                f"Reason={lock_reason}"
            )
            return False, f"Break-even lock acquisition failed: {lock_reason}", None
        ...
        with self._tracking_lock:
            ...
            self._position_tracking[ticket] = tracking
        return True, "Break-even SL applied", final_sl_price
    ...
    def _apply_sweet_spot_lock(self, position: Dict[str, Any], current_profit: float) -> Tuple[bool, str, Optional[float]]:
        ...
        # Apply SL update using unified pipeline (prepare + minimal modify)
        reason = (
            f"Sweet-spot profit locking (${current_profit:.2f} in range "
            f"${self.sweet_spot_min:.2f}-${self.sweet_spot_max:.2f})"
        )
        should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
            ticket, symbol, target_sl, profit_to_lock, reason
        )

        if not should_proceed:
            return False, f"Sweet-spot lock blocked: {error_reason}", None

        final_sl_price = adjusted_sl_price if adjusted_sl_price is not None else target_sl

        lock_acquired, lock, lock_reason = self._acquire_ticket_lock_with_timeout(
            ticket, is_profit_locking=True, force_non_blocking_first=False, is_trailing=False
        )
        if not lock_acquired or lock is None:
            logger.warning(
                f"[SWEET_SPOT_LOCK_FAILED] Ticket={ticket} Symbol={symbol} | "
                f"Reason={lock_reason}"
            )
            return False, f"Sweet-spot lock acquisition failed: {lock_reason}", None
        ...
        with self._tracking_lock:
            ...
            tracking['last_locked_profit'] = current_profit
            self._position_tracking[ticket] = tracking
        return True, f"Sweet-spot lock applied (${current_profit:.2f})", final_sl_price
    ...
    def _apply_trailing_stop(self, position: Dict[str, Any], current_profit: float) -> Tuple[bool, str, Optional[float]]:
        ...
        FIX 6: Trailing Profit Verification
        - Trailing stops activate when profit > $0.10 (trailing_stop_increment_usd)
        - Locks in profit in $0.10 increments, trailing $0.10 behind current price
        - Example: If profit is $0.25, locks in $0.15 (trailing $0.10 behind)
        - Trailing stops have highest priority (TRAILING > PROFIT_LOCK > HARD_SL)
        - This function is called from compute_authoritative_sl() which is lock-free
        - Actual SL modification happens in update_sl_atomic() with proper locking
        ...
        # Apply SL update using unified pipeline (prepare + minimal modify)
        trailing_reason = (
            f"Trailing stop (profit: ${current_profit:.2f}, locking: ${profit_to_lock:.2f})"
        )
        should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
            ticket, symbol, target_sl, profit_to_lock, trailing_reason
        )

        if not should_proceed:
            return False, f"Trailing stop blocked: {error_reason}", None

        final_sl_price = adjusted_sl_price if adjusted_sl_price is not None else target_sl

        # Trailing has highest priority: treat as profit-locking + trailing for lock preemption
        lock_acquired, lock, lock_reason = self._acquire_ticket_lock_with_timeout(
            ticket, is_profit_locking=True, force_non_blocking_first=True, is_trailing=True
        )
        if not lock_acquired or lock is None:
            logger.warning(
                f"[TRAILING_LOCK_FAILED] Ticket={ticket} Symbol={symbol} | "
                f"Reason={lock_reason}"
            )
            return False, f"Trailing lock acquisition failed: {lock_reason}", None
    ...
    def _execute_sl_modify_only(self, ticket: int, symbol: str, target_sl_price: float) -> bool:
        ...
        Args:
            ticket: Position ticket number
            symbol: Trading symbol
            target_sl_price: Target SL price (already validated)
        
        Returns:
            True if MT5.modify_order() succeeded, False otherwise
        """
        lock_start_time = time.time()
        try:
            # STEP 2 FIX: Check position exists right before modify_order to catch race conditions
            position_check = self.order_manager.get_position_by_ticket(ticket)
            if not position_check:
                logger.warning(f"[POSITION_CLOSED_BEFORE_UPDATE] Ticket {ticket} | "
                             f"Position closed before SL update | Target SL: {target_sl_price:.5f}")
                return False
            ...
            # FIX 2: CRITICAL - Immediate verification after modify_order to catch MT5 "success but SL=0" bugs
            # This is a minimal check that doesn't significantly increase lock hold time
            if success:
                # Brief delay for broker to process (minimal - 50ms)
                time.sleep(0.05)
                fresh_position = self.order_manager.get_position_by_ticket(ticket)
                if fresh_position:
                    applied_sl = fresh_position.sl if hasattr(fresh_position, 'sl') else fresh_position.get('sl', 0.0)
                    symbol_info = self.mt5_connector.get_symbol_info(symbol)
                    if symbol_info:
                        point = symbol_info.get('point', 0.00001)
                        # Check if SL was actually applied (within 1 point tolerance)
                        if abs(applied_sl - target_sl_price) >= point:
                            # MT5 reported success but SL not applied - this is a broker bug
                            logger.error(f"[SL_VERIFY_IMMEDIATE_FAIL] Ticket={ticket} Symbol={symbol} | "
                                       f"modify_order() returned success but SL not applied | "
                                       f"Expected: {target_sl_price:.5f} | Applied: {applied_sl:.5f} | "
                                       f"Diff: {abs(applied_sl - target_sl_price):.8f} | "
                                       f"This indicates MT5 broker bug - will trigger retry")
                            return False  # Return False to trigger retry in caller
            
            return success
        except Exception as e:
            logger.error(f"[EXECUTE_SL_MODIFY_ERROR] Ticket={ticket} Symbol={symbol} | "
                        f"Exception in _execute_sl_modify_only: {e}", exc_info=True)
            return False
    
    def _retry_sl_update_with_backoff(self, ticket: int, symbol: str, target_sl: float, max_retries: int = 3) -> bool:
        """
        FIX 2: Fallback retry mechanism for MT5 failures.
        When MT5 returns success but SL is not actually applied, retry with exponential backoff.
        
        Args:
            ticket: Position ticket number
            symbol: Trading symbol
            target_sl: Target SL price
            max_retries: Maximum number of retry attempts (default: 3)
        
        Returns:
            True if SL update succeeded and was verified, False otherwise
        """
        logger.info(f"[FALLBACK_RETRY_START] Ticket={ticket} Symbol={symbol} | "
                   f"Target SL: {target_sl:.5f} | Max retries: {max_retries}")
        
        for attempt in range(max_retries):
            # Exponential backoff: 0.5s, 1.0s, 2.0s
            delay = (2 ** attempt) * 0.5
            if attempt > 0:
                logger.info(f"[FALLBACK_RETRY_DELAY] Ticket={ticket} | Waiting {delay:.1f}s before attempt {attempt+1}/{max_retries}")
                time.sleep(delay)
            
            logger.info(f"[FALLBACK_RETRY_ATTEMPT] Ticket={ticket} Symbol={symbol} | "
                       f"Attempt {attempt+1}/{max_retries} | Target SL: {target_sl:.5f}")
            
            # Check position still exists
            position = self.order_manager.get_position_by_ticket(ticket)
            if not position:
                logger.warning(f"[FALLBACK_RETRY_POSITION_CLOSED] Ticket={ticket} | Position closed during retry")
                return False
            
            # Retry SL update
            success = self._execute_sl_modify_only(ticket, symbol, target_sl)
            if not success:
                logger.warning(f"[FALLBACK_RETRY_MODIFY_FAILED] Ticket={ticket} | "
                             f"Attempt {attempt+1}/{max_retries} | MT5 modify_order returned False")
                continue
            
            # Verify SL was actually applied
            verified = self._verify_sl_applied(ticket, target_sl, max_retries=2, symbol=symbol)
            if verified:
                logger.info(f"[FALLBACK_RETRY_SUCCESS] Ticket={ticket} Symbol={symbol} | "
                           f"Attempt {attempt+1}/{max_retries} | SL verified after fallback retry")
                return True
            else:
                logger.warning(f"[FALLBACK_RETRY_VERIFY_FAILED] Ticket={ticket} | "
                             f"Attempt {attempt+1}/{max_retries} | MT5 returned success but SL still not verified")
        
        logger.error(f"[FALLBACK_RETRY_EXHAUSTED] Ticket={ticket} Symbol={symbol} | "
                    f"All {max_retries} fallback retry attempts failed")
        return False
    
    def _verify_sl_applied(self, ticket: int, target_sl: float, max_retries: int = 3, 
                           symbol: Optional[str] = None, max_verification_time: Optional[float] = None) -> bool:
        """
        CRITICAL FIX: Robust SL Verification with symbol-aware delays and configurable max time.
        After every SL update, verify the broker actually applied it.
        Retry with exponential backoff until confirmation or max time exceeded.
        
        Args:
            ticket: Position ticket number
            target_sl: Target SL price that should be applied
            max_retries: Maximum number of verification attempts (default: 3)
            symbol: Symbol name (optional, for symbol-aware delays)
            max_verification_time: Maximum time in seconds to spend verifying (optional)
        
        Returns:
            True if SL was verified as applied, False otherwise
        """
        verification_start = time.time()
        verification_start_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        ...
        # FIX 1: Symbol-aware initial delay and retry backoff - Enhanced for MT5 broker reality
        # Crypto/indices require 2000ms minimum, forex requires 1000ms minimum
        if symbol and (symbol in ['BTCXAUm', 'DE30m'] or symbol.startswith('BTC') or 'DE30' in symbol):
            initial_delay = 2.0  # 2000ms for crypto/indices (increased from 1000ms)
            retry_delays = [2.0, 3.0, 4.0, 5.0]  # Exponential backoff: 2000ms, 3000ms, 4000ms, 5000ms
        else:
            initial_delay = 1.0  # 1000ms for forex (increased from 500ms)
            retry_delays = [1.0, 2.0, 3.0, 4.0]  # Exponential backoff: 1000ms, 2000ms, 3000ms, 4000ms
        
        logger.info(f"[SL_VERIFY_START] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                   f"Target SL: {target_sl:.5f} | "
                   f"Max retries: {max_retries} | "
                   f"Max time: {max_verification_time or 'unlimited'}s | "
                   f"Initial delay: {initial_delay*1000:.0f}ms | "
                   f"Timestamp: {verification_start_timestamp}")
        ...
        for attempt in range(max_retries):
            ...
            # Check max verification time
            if max_verification_time and elapsed_time >= max_verification_time:
                logger.error(f"[SL_VERIFY_TIMEOUT] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                           f"Verification timeout after {elapsed_time:.2f}s (max: {max_verification_time}s) | "
                           f"Target SL: {target_sl:.5f} may not have been applied | "
                           f"Timestamp: {attempt_timestamp}")
                return False
            
            position = self.order_manager.get_position_by_ticket(ticket)
            if not position:
                logger.warning(f"[SL_VERIFY_FAILED] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                             f"Position not found during verification | "
                             f"Attempt: {attempt + 1}/{max_retries} | "
                             f"Elapsed: {elapsed_time:.2f}s | "
                             f"Timestamp: {attempt_timestamp}")
                if attempt < max_retries - 1:
                    backoff = retry_delays[min(attempt, len(retry_delays) - 1)]
                    logger.debug(f"[SL_VERIFY_BACKOFF] Waiting {backoff*1000:.0f}ms before retry")
                    time.sleep(backoff)
                continue
            
            # Handle both dict and object position formats
            if isinstance(position, dict):
                actual_sl = position.get('sl', 0.0)
                position_symbol = position.get('symbol', '')
            else:
                actual_sl = getattr(position, 'sl', 0.0)
                position_symbol = getattr(position, 'symbol', '')
            
            # Update symbol if we got it from position
            if not symbol and position_symbol:
                symbol = position_symbol
            
            # Get symbol info for point precision and tolerance
            if symbol:
                symbol_info = self.mt5_connector.get_symbol_info(symbol)
                point = symbol_info.get('point', 0.00001) if symbol_info else 0.00001
                # Symbol-aware tolerance: crypto/indices need larger tolerance
                if symbol in ['BTCXAUm', 'DE30m'] or symbol.startswith('BTC') or 'DE30' in symbol:
                    tolerance = max(1.0, point * 200)  # At least 1.0 point, up to 200 points
                else:
                    tolerance = point * 10  # Standard 10 points for forex
            else:
                tolerance = 0.0001  # Default tolerance
            
            sl_diff = abs(actual_sl - target_sl)
            
            if sl_diff <= tolerance:
                elapsed_time = time.time() - verification_start
                logger.info(f"[SL_VERIFY_SUCCESS] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                          f"SL verified: {actual_sl:.5f} (target: {target_sl:.5f}, diff: {sl_diff:.8f}) | "
                          f"Attempt: {attempt + 1}/{max_retries} | "
                          f"Elapsed: {elapsed_time:.2f}s | "
                ...
                logger.warning(f"[SL_VERIFY_RETRY] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                             f"SL mismatch: {actual_sl:.5f} != {target_sl:.5f} (diff: {sl_diff:.8f}) | "
                             f"Attempt: {attempt + 1}/{max_retries} | "
                             f"Elapsed: {elapsed_time:.2f}s | "
                             f"Tolerance: {tolerance:.5f} | "
                             f"Timestamp: {attempt_timestamp}")
        ...
        elapsed_time = time.time() - verification_start
        logger.error(f"[SL_VERIFY_FAILED] Ticket={ticket} Symbol={symbol or 'UNKNOWN'} | "
                    f"SL verification failed after {max_retries} attempts | "
                    f"Elapsed: {elapsed_time:.2f}s | "
                    f"Target SL: {target_sl:.5f} may not have been applied | "
                    f"Timestamp: {datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
        return False
    
    def _validate_and_adjust_sl_for_broker_constraints(self, symbol: str, ticket: int, 
                                                       target_sl_price: float, order_type: str,
                                                       current_bid: float, current_ask: float,
                                                       stops_level: int, point: float,
                                                       symbol_info: Dict[str, Any]) -> Optional[float]:
        """
        STEP 3 FIX: Validate SL against broker constraints and adjust if needed.
        
        Validates:
        - stops_level (minimum distance from current price)
        - Spread constraints
        
        Returns:
            Adjusted SL price if valid, None if no valid SL can be applied
        """
        try:
            if stops_level <= 0:
                # No stops_level constraint - return original SL
                return target_sl_price
            
            min_distance = stops_level * point
            
            # Handle both string and enum order types
            # order_type can be 'BUY', 'SELL', or mt5.ORDER_TYPE_BUY (0), mt5.ORDER_TYPE_SELL (1)
            is_buy_order = (order_type == 'BUY' or 
                          (isinstance(order_type, int) and order_type == 0) or
                          (hasattr(order_type, 'value') and order_type.value == 0))
            
            if is_buy_order:
                # For BUY orders, SL must be below current bid by at least stops_level
                min_allowed_sl = current_bid - min_distance
                if target_sl_price > min_allowed_sl:
                    # SL too close - adjust to minimum allowed
                    adjusted_sl = min_allowed_sl
                    logger.warning(f"[BROKER_CONSTRAINT_ADJUST] {symbol} Ticket {ticket} | "
                                 f"SL violates stops_level: {stops_level} | "
                                 f"Target: {target_sl_price:.5f} > Min allowed: {min_allowed_sl:.5f} | "
                                 f"Adjusted to: {adjusted_sl:.5f}")
                    return adjusted_sl
            else:  # SELL
                # For SELL orders, SL must be above current ask by at least stops_level
                min_allowed_sl = current_ask + min_distance
                if target_sl_price < min_allowed_sl:
                    # SL too close - adjust to minimum allowed
                    adjusted_sl = min_allowed_sl
            ...
            # SL is valid - return original
            return target_sl_price
            
        except Exception as e:
            logger.error(f"[BROKER_CONSTRAINT_VALIDATION_ERROR] {symbol} Ticket {ticket} | "
                        f"Error validating broker constraints: {e}", exc_info=True)
            # On error, return None to block the update (fail-safe)
            return None
    
    def _prepare_sl_update(self, ticket: int, symbol: str, target_sl_price: float,
                          target_profit_usd: float, reason: str) -> Tuple[bool, Optional[str], Optional[float]]:
        """
        CRITICAL: Prepare SL update by doing ALL validation and calculation OUTSIDE the lock.
        ...
        - Call MT5.modify_order() (that happens in _execute_sl_modify_only)
        - Hold any locks
        - Sleep or delay
        ...
        Returns:
            (should_proceed, error_reason, adjusted_sl_price)
            - should_proceed: True if update should proceed, False if blocked
            - error_reason: Reason if blocked, None if proceeding
            - adjusted_sl_price: Adjusted SL price (may differ from target due to StopLevel)
        """
        apply_start_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        logger.debug(f"[{apply_start_timestamp}] 🎯 _prepare_sl_update START | Ticket: {ticket} | Symbol: {symbol} | Target SL: {target_sl_price:.5f}")
        
        # CRITICAL: Check if position still exists before proceeding
        position = self.order_manager.get_position_by_ticket(ticket)
        if not position:
            logger.warning(f"[POSITION_NOT_FOUND] {symbol} Ticket {ticket} | "
                         f"Position not found during SL update - may have been closed | "
                         f"Target SL: {target_sl_price:.5f} | Reason: {reason}")
            return False, "POSITION_NOT_FOUND", None
        
        # PHASE 1 FIX 1.3: Pre-update validation - reject SL if effective loss > risk limit
        # Use Decimal for precise calculations to prevent float rounding errors
        # Add safety margin: cap SL at 95% of risk limit
        current_profit = position.get('profit', 0.0)
        if current_profit < 0:  # Only validate for losing trades
            try:
                # Create test position with target SL to calculate effective SL profit
                test_position = position.copy()
                test_position['sl'] = target_sl_price
                
                # Calculate effective SL profit using existing method
                effective_sl_profit = self.get_effective_sl_profit(test_position)
                
                # Skip validation if no SL yet (float('-inf'))
                if effective_sl_profit == float('-inf'):
                    pass  # No SL yet, skip validation
                else:
                    # PHASE 1 FIX 1.3: Safety margin - cap at 95% of risk limit (using Decimal for precision)
                    # CRITICAL FIX: Allow exactly max_risk_usd for initial SL (strict loss enforcement)
                    # Only apply safety margin for trailing/profit-locking updates, not for initial -$3.00 SL
                    max_risk_decimal = Decimal(str(self.max_risk_usd))
                    current_profit = position.get('profit', 0.0)
                    is_strict_loss_enforcement = (current_profit < 0 and target_profit_usd < 0)
                    ...
                    if is_strict_loss_enforcement:
                        ...
                        safety_margin = Decimal('0.95')  # 95% safety margin
                        max_allowed_loss_decimal = max_risk_decimal * safety_margin
                        max_allowed_loss = float(max_allowed_loss_decimal)
                        tolerance = Decimal('0.01')  # 1 cent tolerance for trailing
                        max_allowed_loss_with_tolerance = float(max_allowed_loss_decimal + tolerance)
                    
                    # Check if effective SL exceeds limit (more negative than allowed)
                    if effective_sl_profit < -max_allowed_loss_with_tolerance:
                        violation_amount = abs(effective_sl_profit - (-max_allowed_loss_with_tolerance))
                        logger.critical(f"🚨 [PHASE 1 FIX 1.3] FAIL-SAFE PRE-VALIDATION REJECTED: {symbol} Ticket {ticket} | "
                                      f"Target SL: {target_sl_price:.5f} | "
                                      f"Effective SL Profit: ${effective_sl_profit:.2f} | "
                                      f"Exceeds limit: ${-max_allowed_loss_with_tolerance:.2f} by ${violation_amount:.2f} | "
                                      f"REJECTING SL UPDATE")
                        return False, f"FAIL-SAFE_PRE-VALIDATION: Effective SL ${effective_sl_profit:.2f} exceeds limit ${-max_allowed_loss_with_tolerance:.2f}", None
                    
                    # Log if close to limit (within 5% of limit)
                    if effective_sl_profit < -max_allowed_loss * 0.95:
                        logger.debug(f"[FAIL-SAFE PRE-VALIDATION OK] {symbol} Ticket {ticket} | "
                                   f"Effective SL: ${effective_sl_profit:.2f} within safety margin of limit ${-max_allowed_loss:.2f}")
            except Exception as e:
                # If validation fails, log but don't block (fail-safe)
                logger.warning(f"[FAIL-SAFE PRE-VALIDATION ERROR] {symbol} Ticket {ticket} | "
                             f"Error validating fail-safe: {e} | Continuing with update")
                # Continue with update (don't block on validation error)
        ...
        try:
            ...
            if symbol_info is None:
                ...
                return False, "SYMBOL_INFO_UNAVAILABLE", None
            ...
            if tick is None:
                ...
                return False, "MARKET_PRICES_UNAVAILABLE", None
            ...
            if tick_bid is None or tick_ask is None or math.isnan(tick_bid) or math.isnan(tick_ask):
                ...
                return False, "INVALID_TICK_DATA", None
            ...
            if not current_position:
                ...
                return False, "POSITION_NOT_FOUND_VALIDATION", None
            ...
            if sl_difference < point * 2:  # Within 2 points = effectively the same
                ...
                return True, None, target_sl_price  # Consider this success - SL is already correct
            ...
            if not is_profitable:
                ...
                if ticket in self._last_sl_price and ticket in self._last_sl_update:
                    ...
                    if last_sl_diff < oscillation_tolerance and time_since_last < min_oscillation_interval:
                        ...
                        return True, None, target_sl_price  # Skip this update to prevent oscillation (already correct)
            else:
            ...
            # STEP 3 FIX: Enhanced broker constraint validation with automatic adjustment
            adjusted_sl_price = self._validate_and_adjust_sl_for_broker_constraints(
                symbol, ticket, target_sl_price, order_type, current_bid, current_ask, 
                stops_level, point, symbol_info
            )
            
            if adjusted_sl_price is None:
                # Validation failed - cannot proceed
                logger.error(f"[SL_DECISION_GATE] Ticket={ticket} Symbol={symbol} | "
                           f"BLOCKED=BROKER_CONSTRAINT_VIOLATION | TargetSL={target_sl_price:.5f} | "
                           f"Reason={reason}")
                return False, "BROKER_CONSTRAINT_VIOLATION", None
            
            # If SL was adjusted, log the adjustment
            if abs(adjusted_sl_price - target_sl_price) > point:
                logger.warning(f"[SL_ADJUSTED_FOR_BROKER] {symbol} Ticket {ticket} | "
                             f"SL adjusted from {target_sl_price:.5f} to {adjusted_sl_price:.5f} | "
                             f"Reason: Broker constraint (stops_level: {stops_level})")
            
            # Continue with original validation logic for backward compatibility
            if stops_level > 0:
                min_distance = stops_level * point
                if order_type == 'BUY':
                    min_allowed_sl = current_bid - min_distance
                    if target_sl_price > min_allowed_sl:
                        ...
                        # adjusted_sl_price already set by validation method
                else:  # SELL
                    min_allowed_sl = current_ask + min_distance
                    if target_sl_price < min_allowed_sl:
                        logger.warning(f"[{validate_timestamp}] [WARNING] SL violates StopLevel: Target {target_sl_price:.5f} < Min allowed {min_allowed_sl:.5f} (stops_level: {stops_level})")
                        adjusted_sl_price = min_allowed_sl
            
            # Validate spread (log warning but don't block)
            if spread > 0:
            ...
            logger.debug(f"[{validate_end_timestamp}] [OK] Validation complete | StopLevel: {stops_level} | Spread: {spread:.5f} | Adjusted SL: {adjusted_sl_price:.5f} (took {(time.time() - validate_start)*1000:.1f}ms)")
            
            # All validation passed - return success with adjusted SL price
            return True, None, adjusted_sl_price
            ...
            return False, "VALIDATION_EXCEPTION", None
        
        # CRITICAL FIX: Lock scope reduction - ALL preparation happens OUTSIDE the lock
        # Call _prepare_sl_update to do all validation and calculation
        should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
            ticket, symbol, target_sl_price, target_profit_usd, reason
        )
        
        if not should_proceed:
            if error_reason == "POSITION_NOT_FOUND" or error_reason == "POSITION_NOT_FOUND_VALIDATION":
                # Position closed - not an error, just log and return
                return False
            # Other blocking reasons (cooldown, delta, etc.) - return False
            return False
        
        # Use adjusted SL price from preparation
        final_sl_price = adjusted_sl_price if adjusted_sl_price is not None else target_sl_price
        ...
        # NOTE: The actual modify_order() call will happen in the caller (update_sl_atomic) INSIDE the lock
        # This method now only does preparation and verification (both outside lock)
        # For backward compatibility, we'll still do a minimal modify here if called directly
        # But the preferred path is: caller calls _prepare_sl_update, then _execute_sl_modify_only inside lock
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # FIX #5: Check position existence before proceeding with SL update
                pre_update_position = self.order_manager.get_position_by_ticket(ticket)
                if not pre_update_position:
                    # Position closed - exit cleanly without errors or retries
                    logger.info(f"[POSITION_CLOSED] {symbol} Ticket {ticket} | Position closed during SL update - exiting cleanly")
                    return False
                ...
                logger.info(f"🔥 SL UPDATE ATTEMPT: Ticket={ticket} Symbol={symbol} OldSL={current_sl:.5f} NewSL={final_sl_price:.5f} Reason={reason}")
                logger.debug(f"[{modify_timestamp}] [JUMP] Placing SL modification | Ticket: {ticket} | Target SL: {final_sl_price:.5f}")
                
                # CRITICAL FIX: Use _execute_sl_modify_only for minimal lock scope
                # This method ONLY calls modify_order - no validation, no delays
                # NOTE: This is called here for backward compatibility, but preferred path is caller using lock directly
                success = self._execute_sl_modify_only(ticket, symbol, final_sl_price)
                ...
                # CRITICAL FIX: Robust SL Verification - verify broker actually applied the SL
                if success:
                    # Get max verification time from config (default: 10 seconds)
                    max_verification_time = self.risk_config.get('sl_verification_max_time_seconds', 10.0)
                    verified = self._verify_sl_applied(
                        ticket, final_sl_price, 
                        max_retries=3, 
                        symbol=symbol,
                        max_verification_time=max_verification_time
                    )
                    if not verified:
                        logger.warning(f"[BROKER_SL_VERIFY_FAILED] Ticket {ticket} Symbol={symbol} | "
                                     f"MT5 returned success but SL not verified | "
                                     f"Target: {final_sl_price:.5f} | Attempting fallback retry mechanism")
                        # FIX 2: Fallback retry mechanism - retry SL update with exponential backoff
                        fallback_success = self._retry_sl_update_with_backoff(ticket, symbol, final_sl_price, max_retries=3)
                        if fallback_success:
                            logger.info(f"[FALLBACK_RETRY_SUCCESS] Ticket {ticket} Symbol={symbol} | "
                                       f"Fallback retry succeeded after MT5 verification failure")
                            success = True  # Mark as success after fallback retry
                        else:
                            logger.error(f"[FALLBACK_RETRY_FAILED] Ticket {ticket} Symbol={symbol} | "
                                        f"Fallback retry failed after {max_retries} attempts")
                            success = False  # Treat as failure to trigger retry loop
                ...
                if success:
                    ...
                    # PHASE 1 FIX 1.2: Fixed verification delay (500ms) - no exponential backoff, no symbol-specific logic
                    # Fixed delay prevents cascading delays and ensures consistent behavior
                    # BUT: For first eligible updates, use minimal delay (never block application)
                    if is_first_eligible:
                        # First eligible: minimal delay (100ms) - never block application
                        verify_delay = 0.1  # 100ms minimum for MT5 to process
                        logger.info(f"[FIRST_ELIGIBLE] Ticket={ticket} Symbol={symbol} | "
                                  f"Using minimal verification delay {verify_delay*1000:.0f}ms (never blocking)")
                    else:
                        # PHASE 1 FIX 1.2: Fixed 500ms delay for all other updates (no symbol-specific logic)
                        verify_delay = self.sl_update_verification_delay  # 500ms (fixed, no exponential backoff)
                        logger.debug(f"[VERIFY_DELAY] Ticket={ticket} Symbol={symbol} | "
                                   f"Using fixed {verify_delay*1000:.0f}ms delay (PHASE 1 FIX 1.2)")
                    ...
                    if fresh_position:
                        ...
                        if sl_diff < tolerance:
                            ...
                            if effective_error < effective_tolerance:
                                ...
                                logger.info(f"[OK] SL UPDATE SUCCESS: Ticket={ticket} Symbol={symbol} NewSL={applied_sl:.5f} TargetSL={final_sl_price:.5f} Reason={reason}")
                                logger.info(f"[{success_timestamp}] [OK] SL APPLIED: {symbol} Ticket {ticket} | "
                                          f"Entry: {entry_price:.5f} | Target: {final_sl_price:.5f} | Applied: {applied_sl:.5f} | "
                                ...
                                self._log_structured_update(
                                    ...
                                    target_sl=final_sl_price, applied_sl=applied_sl,
                            ...
                            logger.info(f"[SL_DECISION_GATE] Ticket={ticket} Symbol={symbol} | "
                                      ...
                                      f"TargetSL={final_sl_price:.5f} AppliedSL={applied_sl:.5f} | "
                                      f"Diff={sl_diff:.5f} Tolerance={tolerance:.5f} | Reason={reason}")
                            logger.warning(f"[WARNING] SL MISMATCH: {symbol} Ticket {ticket} | "
                                         f"Target: {final_sl_price:.5f} | Applied: {applied_sl:.5f} | "
                                         f"Difference: {sl_diff:.5f} (tolerance: {tolerance:.5f})")
                            tracer.trace(
                                function_name="SLManager._apply_sl_update",
                                expected=f"Apply SL update for {symbol} Ticket {ticket}",
                                actual=f"SL price mismatch (target: {final_sl_price:.5f}, applied: {applied_sl:.5f}, diff: {sl_diff:.5f})",
                                status="WARNING",
                                ticket=ticket,
                                symbol=symbol,
                                target_sl=final_sl_price,
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.info(f"[SL_DECISION_GATE] Ticket={ticket} Symbol={symbol} | "
                                  ...
                                  f"TargetSL={final_sl_price:.5f} | Reason={reason}")
                        logger.error(f"[{failure_timestamp}] SL UPDATE FAILED: {symbol} Ticket {ticket} | "
                                   f"Target: {final_sl_price:.5f} | Reason: Verification failed | "
    ...
    def compute_authoritative_sl(self, position: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_profit > self.trailing_increment_usd:  # Profit > $0.10
            # CRITICAL: Check if profit zone updates are enabled
            if not self.profit_zone_updates_enabled:
                logger.debug(f"[PROFIT_ZONE_DISABLED] {symbol} Ticket {ticket} | Trailing stop disabled - skipping (profit: ${current_profit:.2f})")
            else:
                try:
        ...
        if not trailing_result and self.sweet_spot_min <= current_profit <= self.sweet_spot_max:
            # CRITICAL: Check if profit zone updates are enabled
            if not self.profit_zone_updates_enabled:
                logger.debug(f"[PROFIT_ZONE_DISABLED] {symbol} Ticket {ticket} | Sweet spot profit locking disabled - skipping (profit: ${current_profit:.2f})")
            else:
                ...
                if profit_locking_engine:
                    try:
                        ...
                        if profit_locking_success:
                            # CRITICAL FIX #4: ProfitLockingEngine succeeded - apply SL IMMEDIATELY
                            # Get fresh position to check current state
                            ...
                            if fresh_position_after_ple:
                                ...
                                current_profit_check = fresh_position_after_ple.get('profit', 0.0)
                                
                                # Calculate target SL from profit locking engine result
                                # ProfitLockingEngine returns success, but we need to calculate the target SL
                                # The engine should have applied it, but we verify and apply if needed
                                effective_locked = self.get_effective_sl_profit(fresh_position_after_ple)
                                
                                # CRITICAL: If SL was not applied or is incorrect, apply it immediately
                                # Bypass all cooldowns, rate limits, and circuit breakers for profit locks
                                if applied_sl <= 0 or effective_locked < current_profit_check * 0.5:
                                    # SL not applied or insufficient - apply immediately
                                    logger.critical(f"[PROFIT_LOCK_APPLIED_IMMEDIATE] Ticket={ticket} Symbol={symbol} | "
                                              f"ProfitLockingEngine succeeded but SL not applied correctly | "
                                              f"Current SL: {applied_sl:.5f} | Profit: ${current_profit_check:.2f} | "
                                              f"Applying SL immediately via atomic update...")
                                
                                # Calculate target SL for the profit amount
                                entry_price_ple = fresh_position_after_ple.get('price_open', entry_price)
                                order_type_ple = fresh_position_after_ple.get('type', order_type)
                                lot_size_ple = fresh_position_after_ple.get('volume', lot_size)
                                
                                # Calculate target SL to lock the profit
                                target_sl_for_profit = self._calculate_target_sl_price(
                                    entry_price_ple, current_profit_check, order_type_ple, lot_size_ple, symbol_info, position=fresh_position_after_ple
                                )
                                
                                if target_sl_for_profit:
                                    # Apply immediately using atomic update (bypasses all blocking mechanisms)
                                    sl_update_success, sl_update_reason = self.update_sl_atomic(
                                        ticket, fresh_position_after_ple
                                    )
                                    
                                    if sl_update_success:
                                        logger.info(f"[PROFIT_LOCK_APPLIED_IMMEDIATE] Ticket={ticket} Symbol={symbol} | "
                                                  f"SL applied successfully: {target_sl_for_profit:.5f} | "
                                                  f"Reason: {sl_update_reason}")
                                ...
                                if applied_sl > 0:
                                    ...
                                    logger.info(f"[LOCK_SUCCESS] Authoritative Path | Ticket={ticket} Symbol={symbol} "
                                               ...
                                               f"SL: {applied_sl:.5f} | Locked: ${effective_locked:.2f}")
                                else:
                            ...
                            # Track profit-lock failure for alerting
                            self._track_profit_lock_failure(ticket, symbol, profit_locking_reason)
                    except Exception as e:
        ...
        if not trailing_result and not profit_lock_result and current_profit < 0:
            try:
                # CRITICAL FIX #5: Validate current profit before calculating SL
                # If profit already exceeds -$2.00 limit, this is a violation that requires immediate position closure
                # DO NOT close profitable trades - only close when violation occurs
                if current_profit < -self.max_risk_usd:
                    # Profit already exceeds limit - this is a violation
                    violation_msg = (
                        f"[CRITICAL][SL_VIOLATION_DETECTED] Ticket {ticket} Symbol {symbol} | "
                        f"Current profit: ${current_profit:.2f} exceeds -${self.max_risk_usd:.2f} limit | "
                        f"Position must be closed immediately"
                    )
                    logger.critical(violation_msg)
                    
                    # Return violation result - caller should close position
                    hard_sl_result = {
                        'target_sl_price': None,  # No SL update - position must be closed
                        'target_profit_usd': current_profit,
                        'authority_source': 'VIOLATION',
                        'reason': f"SL violation detected: profit ${current_profit:.2f} exceeds -${self.max_risk_usd:.2f} limit",
                        'state': 'VIOLATION',
                        'violation': True,
                        'must_close': True  # Flag indicating position must be closed
                    }
                    result['violations'].append(violation_msg)
                else:
                    # Profit is within limit - calculate HARD_SL normally
        ...
        if authoritative_result:
            ...
            if current_sl > 0:
                ...
                if order_type == 'BUY':
                    ...
                    # FIX 1: Handle None target_sl_price to prevent TypeError
                    target_sl = authoritative_result.get('target_sl_price')
                    if target_sl is not None and current_sl is not None and target_sl < current_sl:
                        would_regress = True
                        violation_msg = f"[CRITICAL][SL_VIOLATION] SL regression attempt: {symbol} Ticket {ticket} | Current SL: {current_sl:.5f} | Target SL: {target_sl:.5f} | Authority: {authoritative_result['authority_source']} | Profit: ${current_profit:.2f}"
                        result['violations'].append(violation_msg)
                        logger.critical(violation_msg)
                else:  # SELL
    ...
    def schedule_initial_sl_application(self, ticket: int, symbol: str, order_type: str, lot_size: float, entry_price: float, delay_seconds: float = 2.5) -> None:
        """
        Schedule initial SL application after stabilization delay.
        
        NEW APPROACH: Trades open WITHOUT SL (null/omitted), then after 0.5-1.0 seconds:
        ...
        This prevents:
        - Early closures due to SL=0 hits
        - Incorrect SL calculation from price fluctuations during order placement
        - Unnecessary SL updates for profitable trades
        
        FIX #1: Reduced delay from 2.0-3.0s to 0.5-1.0s to minimize SL=0.0 window
        ...
        Args:
            ...
            entry_price: Actual entry price of the trade
            delay_seconds: Delay before applying SL (default 0.5 seconds, range 0.5-1.0 seconds)
        """
        # BLOCKER #3 FIX: Reduce delay to ≤100ms to eliminate SL=0.0 window
        # Clamp delay to 0.05-0.1 seconds range (50-100ms maximum)
        delay_seconds = max(0.05, min(0.1, delay_seconds))
        
        logger.info(f"[INITIAL_SL_SCHEDULE] Ticket={ticket} Symbol={symbol} | "
                   f"Scheduling loss-side SL application after {delay_seconds:.1f}s delay | "
                   f"Entry: {entry_price:.5f} | Type: {order_type}")
        
        def apply_initial_sl():
            """Apply initial loss-side SL after delay, only if trade is losing."""
            try:
                # CRITICAL FIX: Check position BEFORE delay to apply SL immediately if losing
                # This prevents early closures due to SL=0.0
                position = self.order_manager.get_position_by_ticket(ticket)
                if not position:
                    logger.warning(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                                 f"Position not found - may have been closed")
                    return
                
                current_sl = position.get('sl', 0.0)
                current_profit = position.get('profit', 0.0)
                
                # If SL is already set, verify it's correct
                if current_sl != 0.0:
                    logger.info(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                              f"SL already set to {current_sl:.5f} (expected 0.0) - verifying...")
                    # Verify effective SL doesn't exceed -$2.00
                    effective_sl_profit = self.get_effective_sl_profit(position)
                    if effective_sl_profit < -self.max_risk_usd:
                        violation = abs(effective_sl_profit - (-self.max_risk_usd))
                        logger.critical(f"[INITIAL_SL_VIOLATION] Ticket={ticket} Symbol={symbol} | "
                                      f"Effective SL ${effective_sl_profit:.2f} exceeds -${self.max_risk_usd:.2f} by ${violation:.2f} | "
                                      f"Correcting immediately...")
                        # Force correction
                        self.update_sl_atomic(ticket, position)
                    else:
                        logger.info(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                ...
                # Small delay allows price to stabilize, but we'll check profit again
                ...
                if not position:
                    logger.warning(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                                 f"Position not found after {delay_seconds:.1f}s delay - may have been closed")
                    return
                
                current_profit = position.get('profit', 0.0)
                current_sl = position.get('sl', 0.0)
                
                # If SL was set during delay, verify it
                if current_sl != 0.0:
                    logger.info(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                              f"SL was set during delay to {current_sl:.5f} - verifying...")
                    effective_sl_profit = self.get_effective_sl_profit(position)
                    if effective_sl_profit < -self.max_risk_usd:
                        violation = abs(effective_sl_profit - (-self.max_risk_usd))
                        logger.critical(f"[INITIAL_SL_VIOLATION] Ticket={ticket} Symbol={symbol} | "
                ...
                # If trade became losing during delay, apply SL now
                if current_profit < 0:
                    logger.info(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                              f"Trade is LOSING (${current_profit:.2f}) after {delay_seconds:.1f}s delay | "
                              f"Applying loss-side SL for -${self.max_risk_usd:.2f}")
                    self._apply_immediate_sl(ticket, symbol, order_type, lot_size, entry_price, position)
                    return
                
                # Trade is profitable - let profit locking handle it
                logger.info(f"[INITIAL_SL] Ticket={ticket} Symbol={symbol} | "
                          f"Trade is PROFITABLE (${current_profit:.2f}) after {delay_seconds:.1f}s | "
                          f"Skipping loss-side SL - profit locking/sweet spot will handle protection")
                return
            except Exception as e:
                logger.error(f"[INITIAL_SL_EXCEPTION] Ticket={ticket} Symbol={symbol} | "
                           f"Exception in apply_initial_sl: {e}", exc_info=True)
        
        # Start thread to apply SL after delay (or immediately if losing)
        thread = threading.Thread(target=apply_initial_sl, daemon=True, name=f"InitialSL-{ticket}")
        thread.start()
        
        logger.debug(f"[INITIAL_SL_SCHEDULE] Ticket={ticket} Symbol={symbol} | "
                    f"Thread started, will check position and apply SL if needed")
    
    def _apply_immediate_sl(self, ticket: int, symbol: str, order_type: str, 
                           lot_size: float, entry_price: float, position: Dict[str, Any]):
        """
        Apply immediate SL for losing trades with SL=0.0.
        
        This method is called when a trade opens losing and needs urgent SL protection.
        It bypasses normal scheduling and applies SL immediately via update_sl_atomic.
        ...
        """
        try:
            logger.info(f"[IMMEDIATE_SL] Ticket={ticket} Symbol={symbol} | "
                       f"Applying immediate SL for losing trade | "
                       f"Entry: {entry_price:.5f} | Profit: ${position.get('profit', 0.0):.2f} | "
                       f"Max risk: -${self.max_risk_usd:.2f}")
            
            # Force immediate SL update via atomic update method
            # This will calculate correct SL based on max_risk_usd (-$3.00)
            # update_sl_atomic uses compute_authoritative_sl which calls _enforce_strict_loss_limit
            # which correctly uses -self.max_risk_usd for losing trades
            success, reason = self.update_sl_atomic(ticket, position)
            
            if success:
                logger.info(f"[IMMEDIATE_SL_SUCCESS] Ticket={ticket} Symbol={symbol} | "
                           f"Immediate SL application completed | Reason: {reason}")
            else:
                logger.error(f"[IMMEDIATE_SL_FAILED] Ticket={ticket} Symbol={symbol} | "
                            f"Immediate SL application failed | Reason: {reason}")
        except Exception as e:
            logger.error(f"[IMMEDIATE_SL_EXCEPTION] Ticket={ticket} Symbol={symbol} | "
                        f"Exception in _apply_immediate_sl: {e}", exc_info=True)
    ...
    def update_sl_atomic(self, ticket: int, position: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        FIX 1: LIVE RELIABILITY - Verification delays increased in this function
        ...
        # PHASE 1 FIX 1.2: Position existence check before SL update (skip if position closed)
        # This prevents errors when positions are closed while SL update is in progress
        fresh_position_check = self.order_manager.get_position_by_ticket(ticket)
        if not fresh_position_check:
            reason = f"Position {ticket} not found - may have been closed"
            logger.warning(f"[POSITION_NOT_FOUND] {symbol} Ticket {ticket} | {reason}")
            # PHASE 1 FIX 1.2: Track position not found count
            with self._verification_lock:
                self._verification_metrics['position_not_found_count'] += 1
            with self._tracking_lock:
                self._last_sl_reason[ticket] = reason
            return False, reason
        
        # Use fresh position data if available (more up-to-date than passed position)
        if fresh_position_check:
            position = fresh_position_check
        ...
        # FIX #3: Check first eligible BEFORE circuit breaker to ensure it's never blocked
        current_profit = position.get('profit', 0.0)
        ...
        # FIX #3: Check if this is the first eligible update BEFORE circuit breaker check
        with self._tracking_lock:
            first_eligible_info = self._first_eligible_update.get(ticket, {})
            first_eligible_state = first_eligible_info.get('state', 'NONE')
            is_first_eligible = (first_eligible_state == 'NONE' or first_eligible_state == 'PENDING')
        
        # FIX #3: Reset circuit breaker on successful profit-locking update OR first eligible update
        ...
        # FIX #3: Check circuit breaker AFTER first eligible check to ensure first eligible always bypasses
        # FIX #3: Determine is_trailing BEFORE circuit breaker check (needed for bypass logic)
        # Note: is_trailing will be determined later from authoritative_result, but we need a preliminary check
        ...
        if ticket in self._ticket_circuit_breaker:
            ...
            if current_time >= disabled_until + self._circuit_breaker_cooldown_seconds:
                ...
                if not is_emergency and not is_profit_locking and not is_first_eligible and not is_trailing_preliminary:
                    ...
                    logger.debug(f"[PAUSE] CIRCUIT BREAKER: {symbol} Ticket {ticket} | "
                               ...
                               f"Emergency: {is_emergency}, Profit-locking: {is_profit_locking}, First-eligible: {is_first_eligible}, Trailing: {is_trailing_preliminary}")
                    # CRITICAL: Set _last_sl_reason even on circuit breaker
                    with self._tracking_lock:
                        self._last_sl_reason[ticket] = reason
                    return False, reason
                else:
                    # Allow emergency, profit-locking, trailing, or first eligible updates even during circuit breaker
                    update_type = 'emergency' if is_emergency else ('trailing' if is_trailing_preliminary else ('profit-locking' if is_profit_locking else 'first-eligible'))
        ...
        # FIX #4: Bypass rate limit for emergency, profit-locking, trailing, and first eligible updates
        consecutive_failures = self._consecutive_failures.get(ticket, 0)
        # Determine if this is a critical update that must bypass rate limiting
        is_critical_update = is_emergency or is_profit_locking or is_first_eligible
        allowed, backoff_delay = self._check_global_rpc_rate_limit(
        ...
        # CRITICAL FIX #5: If violation detected, close position immediately
        # DO NOT close profitable trades - only close when SL violation occurs
        if authoritative_result.get('must_close', False):
            logger.critical(f"🚨 [SL_VIOLATION_CLOSE] Ticket {ticket} | "
                           f"Closing position due to SL violation: {authoritative_result.get('reason', 'Unknown')}")
            
            # Close position immediately (bypass all locks and cooldowns)
            try:
                close_result = self.order_manager.close_position(ticket)
                if close_result:
                    logger.info(f"✅ [SL_VIOLATION_CLOSED] Ticket {ticket} | Position closed successfully")
                    return True, f"Position closed due to SL violation: {authoritative_result.get('reason', 'Unknown')}"
                else:
                    logger.error(f"❌ [SL_VIOLATION_CLOSE_FAILED] Ticket {ticket} | Failed to close position")
                    return False, f"Failed to close position after SL violation"
            except Exception as e:
                logger.error(f"❌ [SL_VIOLATION_CLOSE_EXCEPTION] Ticket {ticket} | Exception: {e}", exc_info=True)
                return False, f"Exception closing position after SL violation: {e}"
        ...
        if has_authoritative_sl and not authoritative_result.get('violations'):
            ...
            # CRITICAL: Also check if this is the first trailing stop update (bypass locks)
            # CRITICAL FIX: Define variables outside lock block for use after
            is_first_eligible = False
            is_first_trailing = False
            with self._tracking_lock:
                first_eligible_info = self._first_eligible_update.get(ticket, {})
                first_eligible_state = first_eligible_info.get('state', 'NONE')
                is_first_eligible = (first_eligible_state == 'NONE' or first_eligible_state == 'PENDING')
                
                # CRITICAL FIX: For trailing stops, also check if this is the first trailing update
                # This ensures first trailing stop bypasses locks immediately
                is_first_trailing = is_trailing and first_eligible_state == 'NONE'
                
                if (is_first_eligible and first_eligible_state == 'NONE') or is_first_trailing:
                    ...
                    if is_first_trailing:
                        logger.info(f"[FIRST_TRAILING] Ticket={ticket} Symbol={symbol} | "
                                  f"First trailing stop update detected | Profit=${current_profit:.2f} | "
                                  f"TargetSL={authoritative_result.get('target_sl_price'):.5f} | "
                                  f"Bypassing all blocking mechanisms and locks")
                    else:
                        logger.info(f"[FIRST_ELIGIBLE] Ticket={ticket} Symbol={symbol} | "
            ...
            # CRITICAL: For trailing stops, force-release stale locks immediately (100ms threshold)
            # This ensures trailing stops are never blocked by stale locks
            if is_trailing:
                lock = self._get_ticket_lock(ticket)
                with self._locks_lock:
                    if ticket in self._lock_hold_times:
                        ...
                        if hold_duration > threshold:
                            logger.critical(f"[TRAILING_PRIORITY] Ticket={ticket} Symbol={symbol} | "
                                          ...
                                          f"Profit=${current_profit:.2f} | Threshold={threshold*1000:.0f}ms")
                            # Check if holder is non-trailing - preempt it
                            holder_info = self._lock_holders.get(ticket)
                            holder_is_trailing = holder_info.get('is_trailing', False) if holder_info else False
                            if not holder_is_trailing:
                                logger.critical(f"[TRAILING_PREEMPTION] Ticket={ticket} | "
                                              f"Trailing stop preempting non-trailing lock | "
                                              f"Holder: {holder_info.get('thread_name', 'Unknown') if holder_info else 'Unknown'}")
                            del self._lock_hold_times[ticket]
                            if ticket in self._lock_holders:
                                del self._lock_holders[ticket]
                            # Try non-blocking acquisition multiple times for trailing
                            max_attempts = 5 if is_first_eligible else 3  # More attempts for first trailing
                            for force_attempt in range(max_attempts):
                                if lock.acquire(blocking=False):
                                    lock.release()
                                    logger.info(f"[TRAILING_PRIORITY] Ticket={ticket} | Stale lock force-released successfully (attempt {force_attempt+1}/{max_attempts})")
                                    break
                                elif force_attempt < max_attempts - 1:
                                    time.sleep(0.01)  # 10ms between attempts
            ...
            # CRITICAL: For first eligible updates and trailing stops, use non-blocking first attempt (immediate)
            lock_acquired, lock, lock_reason = self._acquire_ticket_lock_with_timeout(
                ticket, is_profit_locking=is_profit_locking, force_non_blocking_first=(is_first_eligible or is_trailing), is_trailing=is_trailing
            )
            ...
            if not lock_acquired:
                ...
                if needs_guaranteed_execution or is_first_eligible:
                    ...
                    # CRITICAL: For trailing stops, pass is_trailing flag to retry
                    lock_acquired, lock, lock_reason = self._acquire_ticket_lock_with_timeout(
                        ticket, is_profit_locking=is_profit_locking, force_non_blocking_first=(is_first_eligible or is_trailing), is_trailing=is_trailing
                    )
            ...
            # CRITICAL FIX #1: Lock scope reduction - ALL preparation happens OUTSIDE the lock
            # Prepare SL update (validation, calculation, adjustment) - NO LOCKS HELD
            prepare_start_time = time.time()
            should_proceed, error_reason, adjusted_sl_price = self._prepare_sl_update(
                ticket, symbol, target_sl_price, target_profit_usd, reason_str
            )
            prepare_time = (time.time() - prepare_start_time) * 1000  # ms
            
            if not should_proceed:
                if error_reason == "POSITION_NOT_FOUND" or error_reason == "POSITION_NOT_FOUND_VALIDATION":
                    logger.warning(f"[POSITION_NOT_FOUND] {symbol} Ticket {ticket} | Position not found - may have been closed")
                    return False, "POSITION_NOT_FOUND"
                # Other blocking reasons (cooldown, delta, etc.)
                logger.info(f"[SL_BLOCKED] {symbol} Ticket {ticket} | Reason: {error_reason}")
            ...
            # CRITICAL FIX: Lock scope reduction - lock ONLY during MT5.modify_order() call (<50ms target)
            # All validation, calculation, and verification happen OUTSIDE the lock
            if needs_guaranteed_execution:
                execution_start_time = time.time()
                max_execution_time = 0.25  # 250ms max
                
                # CRITICAL FIX: Lock scope reduction - only lock during MT5 modification
                # All preparation (validation, calculation) already done OUTSIDE lock
                # Lock is acquired ONLY for the MT5.modify_order() call
                # Lock is released IMMEDIATELY after modify_order() returns
                with self._locks_lock:
                    if ticket in self._lock_holders:
                            self._lock_holders[ticket]['is_trailing'] = True
                    
                    logger.info(f"[TRAILING_LOCK_ACQUIRE] Ticket={ticket} Symbol={symbol} | "
                              f"Acquiring lock for trailing stop update | TargetSL={final_sl_price:.5f} | "
                ...
                with lock:
                    ...
                    if lock_acquire_time > 10:
                        logger.warning(f"[LOCK_ACQUIRE_TIME] Ticket={ticket} | Lock acquisition took {lock_acquire_time:.1f}ms")
                    
                    # ONLY call the minimal modify method - no validation, no verification, no delays
                    modify_start = time.time()
                    success = self._execute_sl_modify_only(ticket, symbol, final_sl_price)
                ...
                # CRITICAL FIX: Lock wrapper handles tracking cleanup automatically in __exit__
                
                execution_time = time.time() - execution_start_time
                
                # CRITICAL: Verification happens OUTSIDE the lock (non-blocking)
                if success:
                    # PHASE 1 FIX 1.2: Fixed verification delay (500ms) - no symbol-specific logic
                    # Fixed delay prevents cascading delays and ensures consistent behavior
                    verification_delay = self.sl_update_verification_delay  # 500ms (fixed, no exponential backoff)
                    logger.debug(f"[VERIFY_DELAY] Ticket={ticket} Symbol={symbol} | "
                               f"Using fixed {verification_delay*1000:.0f}ms delay (PHASE 1 FIX 1.2)")
                    time.sleep(verification_delay)
                    # Verify SL was applied
                    fresh_position = self.order_manager.get_position_by_ticket(ticket)
                    if fresh_position:
                        applied_sl = fresh_position.get('sl', 0.0)
                        sl_diff = abs(applied_sl - final_sl_price)
                        symbol_info = self.mt5_connector.get_symbol_info(symbol)
                        point = symbol_info.get('point', 0.00001) if symbol_info else 0.00001
                        tolerance = point * 10  # 1 pip tolerance
                        if sl_diff < tolerance:
                            logger.info(f"[OK] SL VERIFIED: {symbol} Ticket {ticket} | "
                                      f"Applied: {applied_sl:.5f} | Target: {final_sl_price:.5f} | "
                                      f"Diff: {sl_diff:.5f}")
                        else:
                            logger.warning(f"[WARNING] SL VERIFICATION MISMATCH: {symbol} Ticket {ticket} | "
                                         f"Applied: {applied_sl:.5f} | Target: {final_sl_price:.5f} | "
                                         f"Diff: {sl_diff:.5f} (tolerance: {tolerance:.5f})")
                    else:
                        # FIX #3: Position closed during verification - this is not a verification failure
                        # Log as informational rather than warning to distinguish from actual verification failures
                        logger.info(f"[POSITION_CLOSED_DURING_VERIFY] Ticket {ticket} Symbol {symbol} | "
                                   f"Position closed during SL verification | "
                                   f"SL was applied successfully but position closed before verification could complete | "
                                   f"This is normal if position was closed by broker (SL hit, TP hit, or manual close)")
                    
                    # Update tracking
                    with self._tracking_lock:
                        self._last_sl_update[ticket] = datetime.now()
                        self._last_sl_price[ticket] = final_sl_price
                    ...
                    # CRITICAL: Log trailing stop success with timing
                    if is_trailing:
                        logger.info(f"[TRAILING_SUCCESS] Ticket={ticket} Symbol={symbol} | "
                                  f"Trailing stop APPLIED | ExecutionTime={execution_time*1000:.1f}ms | "
                                  f"OldSL={old_sl:.5f} NewSL={final_sl_price:.5f} | "
                                  f"Profit=${current_profit:.2f} Locked=${target_profit_usd:.2f}")
                        
                        system_event_logger.systemEvent("TRAILING_EXECUTED", {
                            "ticket": ticket,
                            "symbol": symbol,
                            "old_sl": old_sl,
                            "new_sl": final_sl_price,
                            "profit": current_profit,
    ...
    def update_all_positions_synchronous(self) -> Tuple[bool, Dict[str, Any]]:
        """
        Update SL for all open positions synchronously.
        
        Returns:
            (success, stats) tuple where:
            - success: True if all positions updated successfully, False otherwise
            - stats: Dictionary with 'failed_tickets' (list) and 'failure_count' (int)
        """
        positions = self.order_manager.get_open_positions()
        failed_tickets = []
        failure_count = 0
        ...
        for position in positions:
            ...
            try:
                ...
                if not success:
                    ...
                    failure_count += 1
                    logger.warning(f"[SYNC_UPDATE_FAILED] Ticket {ticket} | {reason}")
            except Exception as e:
                failed_tickets.append(ticket)
                failure_count += 1
                logger.error(f"[SYNC_UPDATE_EXCEPTION] Ticket {ticket} | Exception: {e}", exc_info=True)
    ...
    def _verify_sl_health(self) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        Verify SL health for all open positions.
        
        Checks each position for violations and returns health status.
        
        Returns:
            (sl_health_ok, violations) tuple where:
            - sl_health_ok: True if no violations detected, False otherwise
            - violations: List of violation dictionaries with 'ticket' and 'type' keys
        """
        violations = []
        positions = self.order_manager.get_open_positions()
        ...
        for position in positions:
            ...
            try:
                # Compute authoritative SL to check for violations
                authoritative_result = self.compute_authoritative_sl(position)
                
                # Detect violations for this position
                violation_strings = self._detect_sl_violations(position, authoritative_result)
                
                # Convert violation strings to violation dicts
                for violation_str in violation_strings:
                    violation_type = 'unknown'
                    if 'SL_VIOLATION' in violation_str:
                        violation_type = 'sl_not_moving'
                    elif 'SL_NOT_APPLIED' in violation_str:
                        violation_type = 'sl_not_applied'
                ...
                # Also check for fail-safe violations (loss limit exceeded)
                current_profit = position.get('profit', 0.0)
                if current_profit < 0:
                    effective_sl_profit = self.get_effective_sl_profit(position)
                    if effective_sl_profit != float('-inf'):
                        violation_threshold = -self.max_risk_usd - getattr(self, '_fail_safe_tolerance', 0.0)
                        if effective_sl_profit < violation_threshold:
                            violations.append({
                                'ticket': ticket,
                                'type': 'loss_limit_exceeded',
                                'message': f"Loss limit exceeded: effective SL ${effective_sl_profit:.2f} < threshold ${violation_threshold:.2f}"
                            })
            
            except Exception as e:
                logger.error(f"[SL_HEALTH_CHECK_EXCEPTION] Ticket {ticket} | Exception: {e}", exc_info=True)
                violations.append({
                    'ticket': ticket,
        ...
        sl_health_ok = len(violations) == 0
        return sl_health_ok, violations
    ...
    def get_effective_sl_profit(self, position: Dict[str, Any]) -> float:
        ...
        if sl_price <= 0 or entry_price <= 0:
            # No SL set yet - return explicit sentinel to indicate "no SL yet" (not a violation)
            # Use float('-inf') to explicitly represent "no SL protection yet" without triggering
            # false fail-safe violations during initial 50-100ms window before SL is applied
            return float('-inf')  # Explicit: no SL yet, not a violation
        
        # FIX B: CRITICAL - Handle edge case where SL is in profit direction but trade is at breakeven
        # This prevents effective SL calculation from returning -$inf when SL is moved into profit
        # while trade profit is near zero (e.g., during profit lock application)
        current_profit = position.get('profit', 0.0)
        current_price = position.get('price_current', 0.0)
        
        # Check if SL is in profit direction (would lock profit if trade were profitable)
        if order_type == 'BUY':
            # For BUY: SL above entry = profit direction
            sl_is_in_profit_direction = sl_price > entry_price
        else:  # SELL
            # For SELL: SL below entry = profit direction
            sl_is_in_profit_direction = sl_price < entry_price
        
        # FIX 2: LIVE RELIABILITY - If SL is in profit direction AND trade is at breakeven (within $0.01),
        # effective SL must equal current profit (not -$inf)
        # REGRESSION GUARD: Effective SL must NEVER be worse than current profit when SL is in profit direction
        # WHY THIS FIX EXISTS: When profit lock moves SL into profit while trade is at breakeven,
        # the standard calculation can produce -$inf, causing false max-risk violations.
        # This edge case handler ensures effective SL = current profit (near zero) instead.
        if sl_is_in_profit_direction and abs(current_profit) < 0.01:
            # FIX 2: LIVE RELIABILITY - Log with clear tag for monitoring
            effective_sl_result = current_profit
            logger.info(f"[EFFECTIVE_SL_EDGE_CASE_HANDLED] Ticket={position.get('ticket', 0)} Symbol={symbol} | "
                       f"SL in profit direction but trade at breakeven | "
                       f"SL: {sl_price:.5f} Entry: {entry_price:.5f} Profit: ${current_profit:.2f} | "
                       f"Using current profit ${effective_sl_result:.2f} as effective SL (prevents -$inf calculation)")
            # REGRESSION GUARD - Assert effective SL is not worse than current profit
            # This ensures the fix works correctly and cannot silently regress
            assert effective_sl_result >= current_profit, \
                f"Effective SL {effective_sl_result:.2f} is worse than current profit {current_profit:.2f} - FIX 2 FAILED"
            # Return current profit (which is near zero) instead of calculating
            # This prevents -$inf calculation errors
            return effective_sl_result
        ...
        if point_value and point_value > 0:
            ...
            # FIX 5: LIVE RELIABILITY - Use effective_entry consistently for both BUY and SELL
            # For SELL, effective_entry equals entry_price, but using it ensures consistency
            if order_type == 'BUY':
                price_diff = effective_entry - sl_price
            else:  # SELL
                price_diff = sl_price - effective_entry
            ...
            if current_profit is not None and current_price > 0:
                # Calculate what contract_size would give us the current profit
                # FIX 5: LIVE RELIABILITY - Use effective_entry consistently for both BUY and SELL
                # This ensures correct contract size reverse-engineering for all order types
                if order_type == 'BUY':
                    current_price_diff = current_price - effective_entry
                else:  # SELL
                    current_price_diff = effective_entry - current_price
    ...
    def fail_safe_check_read_only(self):
        """
        Read-only fail-safe check: Verify all negative P/L trades have SL ≤ -$2.00.
        
        CRITICAL FIX 1: This method is READ-ONLY and does NOT acquire locks or mutate SL.
        It only verifies violations and logs them. Enforcement is handled by synchronous SL update cycle.
        
        This method is called from monitoring threads (TrailingStopMonitor) which must be read-only.
        """
        try:
            positions = self.order_manager.get_open_positions()
            ...
            for position in positions:
                ...
                # Get effective SL (read-only calculation)
                effective_sl_profit = self.get_effective_sl_profit(position)
                
                # CRITICAL FIX: Skip violation check when SL is 0 (no SL yet) - expected during initialization
                # float('-inf') indicates "no SL yet" and should not trigger violations
                if effective_sl_profit == float('-inf'):
                    continue  # No SL set yet - skip violation check (expected during initial window)
                
                # Check if SL is worse than -$2.00 (more negative)
                violation_threshold = -self.max_risk_usd - self._fail_safe_tolerance
                if effective_sl_profit < violation_threshold:
                    violation_amount = abs(effective_sl_profit - (-self.max_risk_usd))
                    
                    # CRITICAL: Only log violation - enforcement handled by synchronous SL update
                    logger.critical(f"🚨 [READ-ONLY] FAIL-SAFE VIOLATION DETECTED: {symbol} Ticket {ticket} | "
                                  f"Current P/L: ${current_profit:.2f} | "
                                  f"Effective SL: ${effective_sl_profit:.2f} | "
                                  f"Exceeds limit: ${-self.max_risk_usd:.2f} by ${violation_amount:.2f} | "
                                  f"Synchronous SL update will enforce correction")
                    # Note: Enforcement happens in synchronous SL update cycle, not here
    ...
    def fail_safe_check(self):
        ...
        NOTE: This method acquires locks and should only be called from synchronous SL update cycle.
        For monitoring threads, use fail_safe_check_read_only() instead.
        """
        try:
            positions = self.order_manager.get_open_positions()
            ...
            for position in positions:
                ...
                # CRITICAL FIX: Add tolerance check to prevent infinite loops
                # Only trigger if violation exceeds tolerance (not just equals limit)
                violation_threshold = -self.max_risk_usd - self._fail_safe_tolerance
                if effective_sl_profit < violation_threshold:
                    violation_amount = abs(effective_sl_profit - (-self.max_risk_usd))
                    
                    # CRITICAL FIX: Check cooldown to prevent infinite loops
                    current_time = time.time()
                    cooldown_until = self._fail_safe_cooldown.get(ticket, 0)
                    if current_time < cooldown_until:
                        remaining = cooldown_until - current_time
                        logger.debug(f"[FAIL-SAFE COOLDOWN] Ticket {ticket} Symbol {symbol} | "
                                   f"Fail-safe correction in cooldown: {remaining:.2f}s remaining | "
                                   f"Effective SL: ${effective_sl_profit:.2f} | Violation: ${violation_amount:.2f}")
                        continue  # Skip this ticket during cooldown
                    ...
                    for attempt in range(max_retries):
                        ...
                        if success:
                            ...
                            # CRITICAL FIX: Set cooldown after successful correction
                            self._fail_safe_cooldown[ticket] = current_time + self._fail_safe_cooldown_duration
                            break
                        else:
                            if attempt < max_retries - 1:
                                ...
                                # CRITICAL FIX: Set cooldown even on failure to prevent infinite loops
                                self._fail_safe_cooldown[ticket] = current_time + self._fail_safe_cooldown_duration
                elif effective_sl_profit <= -self.max_risk_usd:
                    # Within tolerance - log but don't trigger correction
                    violation_amount = abs(effective_sl_profit - (-self.max_risk_usd))
                    if violation_amount <= self._fail_safe_tolerance:
                        logger.debug(f"[FAIL-SAFE OK] Ticket {ticket} Symbol {symbol} | "
                                   f"Effective SL: ${effective_sl_profit:.2f} within tolerance (${self._fail_safe_tolerance:.2f}) of limit ${-self.max_risk_usd:.2f}")
    ...
    def cleanup_closed_position(self, ticket: int):
        ...
        with self._tracking_lock:
            ...
            # Clean up fail-safe cooldown
            if ticket in self._fail_safe_cooldown:
                del self._fail_safe_cooldown[ticket]
    ...
    def start_sl_worker(self, watchdog=None):
        ...
        """
        # CRITICAL FIX: Ensure attribute exists (defensive check for initialization issues)
        if not hasattr(self, '_sl_worker_running'):
            logger.error("[ERROR] _sl_worker_running attribute not initialized - SLManager __init__ may have failed")
            self._sl_worker_running = False
            self._sl_worker_thread = None
            self._sl_worker_shutdown_event = threading.Event()
            self._sl_worker_interval = 0.5  # Default 500ms
            self._sl_worker_last_position_count = 0
            self._sl_worker_last_active_tickets = 0
    ...
    def stop_sl_worker(self):
        ...
        # CRITICAL FIX: Ensure attribute exists (defensive check for initialization issues)
        if not hasattr(self, '_sl_worker_running'):
            logger.warning("[WARNING] _sl_worker_running attribute not initialized - SLManager may not be fully initialized")
            return
    ...
    def _background_worker_loop(self):
        ...
        FIX 2: SLBackgroundWorker is READ-ONLY for fail-safe checks.
        - Uses fail_safe_check_read_only() instead of fail_safe_check()
        - Does NOT acquire per-ticket SL locks
        - Prevents lock contention with SLWorker thread
        - All SL mutations are handled EXCLUSIVELY by SLWorker thread
        """
            ...
            try:
                ...
                try:
                    ...
                    if task_type == 'fail_safe_check':
                        # FIX 2: SLBackgroundWorker must be read-only - use read-only fail-safe check
                        # This prevents lock contention between SLBackgroundWorker and SLWorker
                        # Only SLWorker thread should call fail_safe_check() (which acquires locks)
                        # SLBackgroundWorker uses read-only version to avoid lock acquisition
                        try:
                            self.fail_safe_check_read_only()
                        except Exception as e:
                            logger.error(f"Error in background read-only fail-safe check: {e}", exc_info=True)
        ...
        try:
            ...
            while self._sl_worker_running and not self._sl_worker_shutdown_event.is_set():
                ...
                if should_log_debug:
                    ...
                    # PHASE 1 FIX 1.2: Maximum time budget per loop iteration (prevent infinite delays)
                    # If loop has already taken >1 second, skip remaining positions to prevent cascading delays
                    max_loop_time_budget_ms = 1000.0  # 1 second max per loop iteration
                    ...
                    with self._verification_lock:
                        self._verification_metrics['worker_loop_durations'].append(loop_duration)
                        # Keep only last 1000 measurements to prevent memory growth
                        if len(self._verification_metrics['worker_loop_durations']) > 1000:
                            self._verification_metrics['worker_loop_durations'].pop(0)
                    with self._timing_lock:
            ...
            # CRITICAL FIX: Clean up all locks held by this thread before exiting
            # This MUST run even on fatal exceptions to prevent orphaned locks
            try:
                self._cleanup_thread_locks()
            except Exception as cleanup_error:
                logger.error(f"[THREAD_CLEANUP] Error during lock cleanup on exception: {cleanup_error}", exc_info=True)
            
            logger.info(f"[THREAD_STOP] {thread_name} pid={pid} tid={tid} reason=exception_shutdown")
            # This prevents orphaned locks that cause "Holder: No holder info" after thread restart
            try:
                self._cleanup_thread_locks()
            except Exception as cleanup_error:
    ...
    def _check_global_rpc_rate_limit(self, is_emergency: bool = False, consecutive_failures: int = 0, 
        ...
        with self._global_rpc_lock:
            ...
            # FIX #4: Critical updates bypass rate limit: emergency, profit-locking, trailing, first eligible
            is_critical_bypass = is_emergency or consecutive_failures >= 2 or is_profit_locking or is_trailing or is_first_eligible
            
            if is_critical_bypass:
                # Critical update: apply short exponential backoff to avoid broker flooding
                recent_critical_count = sum(1 for ts in self._global_rpc_timestamps if current_time - ts < 0.1)  # Last 100ms
                if recent_critical_count > 5:  # More than 5 critical updates in 100ms
                    backoff = self._emergency_backoff_base * (2 ** min(recent_critical_count - 5, 3))  # Max 400ms
                    logger.debug(f"[WARNING] Critical update backoff: {backoff*1000:.0f}ms (recent critical: {recent_critical_count})")
                    return True, backoff
    ...
    def get_worker_status(self) -> Dict[str, Any]:
        """Get SL worker status (synchronous system - no worker thread)."""
        # For synchronous system, always report as running (available in run_cycle)
        # Get current position count for status
        try:
            positions = self.order_manager.get_open_positions()
            current_position_count = len(positions)
            current_active_tickets = [p.get('ticket') for p in positions if p.get('ticket')]
        except Exception:
            current_position_count = self._sl_worker_last_position_count
            current_active_tickets = []
        
        return {
            'running': True,  # Synchronous system is always available
            'thread_alive': False,  # No worker thread in synchronous system
            'system_type': 'synchronous',  # Indicate this is synchronous system
            'last_position_count': current_position_count,
            'last_active_tickets': len(current_active_tickets),
    ...
    def get_verification_metrics(self) -> Dict[str, Any]:
        ...
        with self._verification_lock:
            ...
            # PHASE 1 FIX 1.2: Calculate worker loop performance metrics
            worker_durations = metrics.get('worker_loop_durations', [])
            avg_loop_duration = sum(worker_durations) / len(worker_durations) if worker_durations else 0.0
            max_loop_duration = max(worker_durations) if worker_durations else 0.0
            ...
            return {
                ...
                # PHASE 1 FIX 1.2: Additional performance metrics
                'worker_loop_avg_duration_ms': avg_loop_duration,
                'worker_loop_max_duration_ms': max_loop_duration,
                'worker_loop_target_ms': 50.0,
                'worker_loop_meets_target': avg_loop_duration < 100.0,
                'sl_update_timeouts': metrics.get('sl_update_timeouts', 0),
                'circuit_breaker_activations': metrics.get('circuit_breaker_activations', 0),
                'position_not_found_count': metrics.get('position_not_found_count', 0),
    ...
    def reset_verification_metrics(self):
        ...
        with self._verification_lock:
            self._verification_metrics = {
                ...
                # PHASE 1 FIX 1.2: Include new metrics in reset
                'worker_loop_durations': [],
                'sl_update_timeouts': 0,
                'circuit_breaker_activations': 0,
                'position_not_found_count': 0
            }
